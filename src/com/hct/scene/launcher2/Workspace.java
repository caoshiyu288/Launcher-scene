...
import com.hct.scene.launcher2.FolderIcon.FolderRingAnimator;
import com.hct.scene.launcher2.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
import com.hct.scene.launcher2.PagedView.IND_STYLE;
...
public class Workspace extends SmoothPagedView
    ...
    private static float CAMERA_DISTANCE = 1920;//5000
    private static int DEGREE = 50;
    private static int DEGREE_FACTOR = 15;
    
    //caoshiyu add for rotation when sliding
    Workspace.ZInterpolator mZInterpolator = new Workspace.ZInterpolator(0.5f);
    private static float TRANSITION_SCALE_FACTOR = 0.8f;//0.74f
    private static float TRANSITION_ZOOM_FACTOR = 0f;//0.74f
    private AccelerateInterpolator mAlphaInterpolator = new AccelerateInterpolator(0.2f);
    private DecelerateInterpolator mLeftScreenAlphaInterpolator = new DecelerateInterpolator(0.2f);//4
    private Rotation3DInterpolator mRotate3DInterpolator = new Workspace.Rotation3DInterpolator(DEGREE, DEGREE_FACTOR);
	...
	/* added by hct lijunyi, 20121011 */
	final private int mPointerOffsetX = 10, mPointerOffsetY = 10;
	...
	/* end of added by hct lijunyi, 20121011 */
    ...
    enum State { NORMAL, SPRING_LOADED, SMALL, ZOOM};
    ...
    protected AnimatorListener mChangeStateAnimationListener;
    ...
    protected final Paint mExternalDragOutlinePaint = new Paint();
    ...
    protected int mUnreadMarginRight = 0;
    
    protected float mDotIndTranslateY;
    
    //caoshiyu add for hotseat animation
    private float mOldHotseatTranslationX;
    private float mOldHotseatTranslationY;
    private float mNewHotseatTranslationX;
    private float mNewHotseatTranslationY;
    private float mOldHotseatAlpha;
    private float mNewHotseatAlpha;
    private float mOldHotseatScaleX;
    private float mNewHotseatScaleX;
    private float mOldHotseatScaleY;
    private float mNewHotseatScaleY;
    
    //caoshiyu add for dotind animation
    private float mOldDotIndTranslationX;
    private float mOldDotIndTranslationY;
    private float mNewDotIndTranslationX;
    private float mNewDotIndTranslationY;
    private float mOldDotIndAlpha;
    private float mNewDotIndAlpha;
    private float mOldDotIndScaleX;
    private float mNewDotIndScaleX;
    private float mOldDotIndScaleY;
    private float mNewDotIndScaleY;
    
    final static String WORKSPACE_EFFECT_KEY = "workspace_effect_key";
    /**
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        a.recycle();
        initCellCountXY(cellCountX, cellCountY);
    ...
    protected void initCellCountXY(int x, int y){
    	LauncherModel.updateWorkspaceLayoutCells(x, y);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_POINTER_DOWN:{
        	if (ev.getPointerCount() > 1) {
				...
				mTouchState = TOUCH_STATE_SHOW_THUMB;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
		case MotionEvent.ACTION_POINTER_UP: {
		if (mTouchState == TOUCH_STATE_SHOW_THUMB) {
			mPointUp1.set(ev.getX(0), ev.getY(0));
			mPointUp2.set(ev.getX(1), ev.getY(1));
			...
			if (((diffDownX - diffUpX) >= mPointerOffsetX) && ((diffDownY - diffUpY) >= mPointerOffsetY)) {
    ...
    protected boolean isScrollingIndicatorEnabled() {
        return mState != State.SPRING_LOADED && isImgInd();
    }

    @Override
	protected IND_STYLE initScrollIndStyle() {
		return IND_STYLE.DOT_STYLE;
	}


    protected void onPageBeginMoving() {
        super.onPageBeginMoving();
        Log.d("scroll3D", "workspace---onPageBeginMoving,mScrollEffectBegin="+mScrollEffectBegin);
        if(!mScrollEffectBegin){
        	restorePageStatus();
        	mScrollEffectBegin = true;
        }
    ...
    private float wallpaperOffsetForCurrentScroll() {
		...
		// >>hct:caoshiyu add
		if (mCircleFlag) {
			final int totalDistance = getScaledMeasuredWidth(getChildAt(0))
					+ mPageSpacing;
			if (getScrollX() > mMaxScrollX) {
				// adjustedScrollX = mMaxScrollX - (getScrollX() - mMaxScrollX);
				scrollProgress = 1 - ((getScrollX() - mMaxScrollX) / (totalDistance * 1.0f));
			} else if (getScrollX() < 0) {
				// adjustedScrollX = -getScrollX();
				scrollProgress = (-getScrollX() / (totalDistance * 1.0f));
			}
		}
		// <<hct:end of add
...
//                cl.fastInvalidate();
    ...
    protected void screenScrolledStandardUI(int screenCenter) {
        if (mIsSwitchingState && (mOverScrollX < 0 || mOverScrollX > mMaxScrollX)){
            ...
            //caoshiyu add
            if(mState == State.NORMAL && !mIsSwitchingState){
//            	scroll3D(screenCenter);
            	scroll3D(screenCenter, Launcher.mWorkspaceEffect);
            }
            ...
            if (v != null) {
                ...
                if(scrollProgress>0){
                	float interpolatedProgress =
                			mZInterpolator.getInterpolation(Math.abs(Math.max(scrollProgress, 0)));
                ...
                Log.d(TAG, "scroll3D-scale=="+scale+",i==="+i+",scrollProgress=="+scrollProgress);
//                float translationX = Math.min(0, scrollProgress) * v.getMeasuredWidth();
                float rotationY = DEGREE * scrollProgress; 
//                float rotationY;
//                if(scrollProgress >0){
//                	rotationY = (DEGREE - DEGREE_FACTOR)*scrollProgress + DEGREE_FACTOR*1.0f;
//                }else{
//                	rotationY = (DEGREE - DEGREE_FACTOR)*scrollProgress - DEGREE_FACTOR*1.0f;
//                }
//                rotationY = mRotate3DInterpolator.getInterpolation(scrollProgress);
                if(scrollProgress == 1.0 || scrollProgress == -1.0 || scrollProgress == 0){
                	v.setBackgroundDrawable(null);
                	rotationY = DEGREE * scrollProgress;
                }else{
                	v.setBackgroundResource(R.drawable.home_scroll);
                }
                float alpha;
...
//                    alpha = mLeftScreenAlphaInterpolator.getInterpolation(1 - scrollProgress);
                	alpha = mAlphaInterpolator.getInterpolation(1 - scrollProgress);
                }
                int bkg_alpha = (int)(Math.abs(scrollProgress)*255.0f);
...
//                v.setTranslationX(translationX);
                ...
                if(v.getBackground() != null){
                	v.getBackground().setAlpha(bkg_alpha);
                }
...
//                }
    ...
    void scroll3D(int screenCenter, int style){
    	...
    	case 0:
    		...
    		if(mScrollEffectBegin){
    			...
    			mLastScrollEffectStyle = random;
    			mScrollEffectBegin = false;
    			scroll3D(screenCenter, random);
    		}else{
    			scroll3D(screenCenter, mLastScrollEffectStyle);
    		}
    		break;
    	case 1:
    		//Classic
    	...
    	case 2:
    		//Wave
    		waveScroll(screenCenter);
    		break;
    	case 3:
    		//Fade
    		fadeScroll(screenCenter);
    		break;
		case 4:
			//Flip
			flipScroll(screenCenter);
			break;
		case 5:
			//Windmill
			windmillScroll(screenCenter);
			break;
		case 6:
			//Cube
			cubeScroll(screenCenter);
			break;
		case 7:
			//Photowall
			photowallScroll(screenCenter);
			break;
		case 8:
			//Push
			pushScroll(screenCenter);
			break;
		case 9:
			//Up and down
			upDownScroll(screenCenter);
			break;
		default:
			break;
		}
    ...
    public void classicScroll(){
    	
    }
    public void waveScroll(int screenCenter){
    	final int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            View v = getPageAt(i);
            if (v != null) {
                ...
                float rotationY = DEGREE * scrollProgress; 
                int bkg_alpha = (int)(Math.abs(scrollProgress)*255.0f);
                if(scrollProgress>0 && scrollProgress < 1f){
                	float interpolatedProgress =
                			mZInterpolator.getInterpolation(Math.abs(Math.max(scrollProgress, 0)));
                	scale = (1 - interpolatedProgress) +
                			interpolatedProgress * TRANSITION_SCALE_FACTOR;
                	...
                	if(v.getBackground() == null)
                		v.setBackgroundResource(R.drawable.home_scroll);
                }else if(scrollProgress < 0 && scrollProgress > -1f){
                	float interpolatedProgress =
                			mZInterpolator.getInterpolation(Math.abs(Math.min(scrollProgress, 0)));
                	scale = (1 - interpolatedProgress) +
                			interpolatedProgress * TRANSITION_SCALE_FACTOR;
                ...
                Log.d(TAG, "scroll3D-wave=="+scale+",i==="+i+",scrollProgress=="+scrollProgress);
    ...
    public void fadeScroll(int screenCenter){
    	final int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            View v = getPageAt(i);
            if (v != null) {
...
//                }
    ...
    public void flipScroll(int screenCenter){
    	final int childCount = getChildCount();
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
			final int childWidth = getScaledMeasuredWidth(v);
			final int childHeight = v.getMeasuredHeight();
            if (v != null) {
                float scrollProgress = getScrollProgress(screenCenter, v, i);
                float rotationY = - 180 * scrollProgress;
                float translateX = translateWidth * scrollProgress;
                float alpha = 0f;
                int bkg_alpha = 0;
                if(scrollProgress < 0 && scrollProgress > - 0.5f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                    bkg_alpha = (int)(Math.abs(scrollProgress)*255.0f);
                    if(v.getBackground() == null)
                    	v.setBackgroundResource(R.drawable.home_scroll);
                } else if(scrollProgress > 0 && scrollProgress < 0.5f){
                	alpha = mAlphaInterpolator.getInterpolation(1 - scrollProgress);
                	bkg_alpha = (int)(scrollProgress*255.0f);
                	if(v.getBackground() == null)
                		v.setBackgroundResource(R.drawable.home_scroll);
                } else if(scrollProgress == 0f || scrollProgress == 1f || scrollProgress == -1f){
                	alpha = 1f;
                	rotationY = 0f;
                	bkg_alpha = 0;
                	translateX = 0f;
                ...
                Log.d(TAG, "scroll3D-flip=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress);
                v.setCameraDistance(mDensity * CAMERA_DISTANCE);
                v.setPivotX(childWidth/2);
                v.setPivotY(childHeight/2);
                v.setRotationY(rotationY);
    ...
    public void windmillScroll(int screenCenter){
    	final int childCount = getChildCount();
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
            ...
            if (v != null) {
                float scrollProgress = getScrollProgress(screenCenter, v, i);
                float rotation = - 45 * scrollProgress;
                float alpha = 0f;
                if(scrollProgress < 0 && scrollProgress > -1f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                } else if(scrollProgress > 0 && scrollProgress < 1f){
                ...
                Log.d(TAG, "scroll3D-1=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress);
                v.setCameraDistance(mDensity * CAMERA_DISTANCE);
                v.setPivotX(childWidth/2);
                v.setPivotY(childHeight);
                v.setRotation(rotation);
                v.setAlpha(alpha);
            }
		}
    }
    public void cubeScroll(int screenCenter){
    	final int childCount = getChildCount();
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
            ...
            if (v != null) {
                ...
                float alpha = 0f;
                int bkg_alpha = 0;
                float privotX = childWidth / 2.0f * (1+scrollProgress);
                if(scrollProgress < 0 && scrollProgress > -1f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                    bkg_alpha = (int)(Math.abs(scrollProgress)*255.0f);
                    if(v.getBackground() == null)
                ...
                Log.d(TAG, "scroll3D-cube=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress);
                v.setCameraDistance(mDensity * CAMERA_DISTANCE);
                v.setPivotX(privotX);
                v.setPivotY(childHeight/2);
                v.setRotationY(rotationY);
    ...
    public void photowallScroll(int screenCenter){
    	final int childCount = getChildCount();
    	final int halfScreenSize = getMeasuredWidth() / 2;
		final float divideProgress = 0.5f;
		final int degree = 60;
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
            ...
            if (v != null) {
                float scrollProgress = getScrollProgress(screenCenter, v, i);
                float rotationY = degree * 1.0f;
                float alpha = 0f;
                float maxScale = 1.5f;
                float minScale = 0.5f;
                float scale = 1.0f;
                float scale1 = 1.0f;
                float pivotX = childWidth/2;
                int direction = screenCenter - (getChildOffset(i) - getRelativeChildOffset(i) + halfScreenSize);
                if(scrollProgress < 0 && scrollProgress > -1f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                    if(mLeftMoving && scrollProgress > - divideProgress){
                    	rotationY = (degree/(divideProgress)) * (Math.abs(scrollProgress));
                    }else if(mRightMoving && scrollProgress <= - divideProgress){
                    	rotationY = (degree/(1f-divideProgress)) * (1-(Math.abs(scrollProgress)));
                    }

                	float interpolatedProgress =
                			mZInterpolator.getInterpolation(Math.abs(Math.min(scrollProgress, 0)));
//                	scale1 = (1 - interpolatedProgress) +
//                			interpolatedProgress * TRANSITION_SCALE_FACTOR;
//                    Log.d(TAG, "iiiiiiiiii<<<<<<<<0=="+i+",scale1==="+scale1);
                    scale = (1-(Math.abs(scrollProgress))) * 0.5f + 0.5f;
                    pivotX = 0;
                } else if(scrollProgress > 0 && scrollProgress < 1f){
                	alpha = mAlphaInterpolator.getInterpolation(1 - scrollProgress);
                	if(mLeftMoving && scrollProgress >= divideProgress){
                		rotationY = (degree/(1f-divideProgress)) * (1 - scrollProgress);
                	}else if(mRightMoving && scrollProgress < divideProgress){
                		rotationY = (degree/(divideProgress)) * scrollProgress;
                	}
                    float interpolatedProgress =
                			mZInterpolator.getInterpolation(Math.abs(Math.max(scrollProgress, 0)));
//                    scale1 = (1 - interpolatedProgress) +
//                			interpolatedProgress * TRANSITION_SCALE_FACTOR;
//                	Log.d(TAG, "iiiiiiiiii>>>>>>>0=="+i+",scale1==="+scale1);
                	 scale = 1.0f*scrollProgress + 1.0f;
                	 pivotX = childWidth;
                } else {
                	alpha = 1f;
                	rotationY = 0f;
                ...
                Log.d(TAG, "scroll3D-photowall=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress+",rotationY=="+rotationY);
                v.setCameraDistance(mDensity * 8000);
                v.setPivotX(pivotX);
                v.setPivotY(childHeight/2);
                v.setRotationY(rotationY);
                v.setScaleX(scale);
    ...
    public void pushScroll(int screenCenter){
    	final int childCount = getChildCount();
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
            ...
            if (v != null) {
                ...
                float alpha = 0f;
                float scale = 1.0f;
                float privotX = childWidth;
                float privotY = childHeight/2;
                if(scrollProgress < 0 && scrollProgress > -1f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                    scale = 1.0f*(1 - Math.abs(scrollProgress));
                } else if(scrollProgress > 0 && scrollProgress<1f){
                	alpha = mAlphaInterpolator.getInterpolation(1 - scrollProgress);
                	scale = 1.0f*(1 - Math.abs(scrollProgress));
                	privotX = 0;
                ...
                Log.d(TAG, "scroll3D-push=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress);
//                v.setCameraDistance(mDensity * CAMERA_DISTANCE);
                v.setPivotX(privotX);
                v.setPivotY(privotY);
                v.setTranslationX(translateX);
    ...
    public void upDownScroll(int screenCenter){
    	final int childCount = getChildCount();
		final float TRANSLATE_FACTOR = getMeasuredHeight()/2;
		for (int i = 0; i < childCount; i++) {
			View v = getPageAt(i);
			final int translateWidth = getScaledMeasuredWidth(v) + mPageSpacing * 2;
			final int childWidth = getScaledMeasuredWidth(v);
...
//                float rotationY = - 180 * scrollProgress;
                float translateY = 0;
                float alpha = 0f;
                float privotX = childWidth/2;
                float privotY = childHeight/2;
                if(scrollProgress < 0 && scrollProgress>-1f){
                    alpha = mAlphaInterpolator.getInterpolation(1 - Math.abs(scrollProgress));
                    translateY = Math.abs(scrollProgress) * TRANSLATE_FACTOR;
                } else if(scrollProgress > 0 && scrollProgress < 1f){
                	alpha = mAlphaInterpolator.getInterpolation(1 - scrollProgress);
                	translateY = scrollProgress * TRANSLATE_FACTOR;
                } else {
                	alpha = 1f;
                	translateY = 0f;
                }
                Log.d(TAG, "scroll3D-updown=="+alpha+",i==="+i+",scrollProgress=="+scrollProgress);
//                v.setCameraDistance(mDensity * CAMERA_DISTANCE);
                v.setPivotX(privotX);
                v.setPivotY(privotY);
//                v.setTranslationX(translateX);
...
//                v.setScaleX(scale);
	...
	protected void restorePageStatus() {
    	if(mState == State.SPRING_LOADED) return;
		// restore the scale, alpha, and rotation status
    	for (int i = 0; i < getChildCount(); i++) {
    		View v = getPageAt(i);
    		int pageWidth = v.getMeasuredWidth();
            int pageHeight = v.getMeasuredHeight();
...
//                }
    ...
    protected void updateChildrenLayersEnabled() {
...
//                    ((ViewGroup)getChildAt(i)).setChildrenLayersEnabled(false);
    ...
    static class Rotation3DInterpolator implements Interpolator{
    	private final int mDegreeFactor1;
    	private final int mDegreeFactor2;
		public Rotation3DInterpolator(int degree1, int degree2) {
			mDegreeFactor1 = degree1;
			mDegreeFactor2 = degree2;
			
		}
		@Override
		public float getInterpolation(float input) {
			if(input > 0){
				return (mDegreeFactor2-mDegreeFactor1)*(input*input - 2*input) + mDegreeFactor2*1.0f;
			}else{
				return (mDegreeFactor1 - mDegreeFactor2)*(input*input + 2*input) - mDegreeFactor2*1.0f;
			}
		}
    ...
    */
    ...
    private void initAnimationArrays() {
    	final int childCount = getMaxPageNum();  /* modify by hct, 20121011 */
    ...
    protected int getMaxPageNum(){
    	return Launcher.WORKSPACE_MAX_PAGE_NUM;
    }
    ...
    void changeState(final State state, boolean animated, int delay) {
        ...
        Log.d("furniture", "changeState---state=="+state);
        if (mFirstLayout) {
        ...
        final boolean oldStateIsZoom = (oldState == State.ZOOM);
        mState = state;
        final boolean stateIsNormal = (state == State.NORMAL);
        final boolean stateIsSpringLoaded = (state == State.SPRING_LOADED);
        final boolean stateIsSmall = (state == State.SMALL);
        final boolean stateIsZoom = (state == State.ZOOM);
        ...
        if (state != State.NORMAL) {
            ...
            if (oldStateIsNormal && stateIsSmall) {
            	Log.d("furniture", "isSmall~~~~~~~~~~~~~");
                zoomIn = false;
                setLayoutScale(finalScaleFactor);
                updateChildrenLayersEnabled();
                //caoshiyu add
                translationY = - 0.3f * this.getMeasuredHeight();
                mDotIndTranslateY = translationY;
            } else if(oldStateIsNormal && stateIsZoom){
            	Log.d("furniture", "isSmall~~~~~~~"+finalScaleFactor);
            	setLayoutScale(finalScaleFactor);
            	updateChildrenLayersEnabled();
            }else {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if((oldStateIsNormal && stateIsZoom) ||
            		(oldStateIsZoom && stateIsNormal)){
            	cl.setPivotX(cl.getMeasuredWidth() / 2.0f);
            	cl.setPivotY(TRANSITION_ZOOM_FACTOR * cl.getMeasuredHeight());
            }
            if(oldStateIsZoom && stateIsNormal){
            	cl.setZoomMode(false);
            }else if(oldStateIsNormal && stateIsZoom){
            	cl.setZoomMode(true);
            }
            ...
            }else if(oldStateIsNormal && stateIsSmall){
                if (i == mCurrentPage || !animated) {
                    finalAlpha = 0.0f;
                    finalAlphaMultiplierValue = 0.0f;
                } else {
                    initialAlpha = 0f;
                    finalAlpha = 0f;
                }
        ...
        //caoshiyu add for hotseat
        /*
        final Hotseat hotseat = mLauncher.getHotseat();
        if(hotseat != null && hasHotseat()){
        	if(animated){
        		mOldHotseatTranslationY = hotseat.getTranslationY();
        		mOldHotseatScaleX = hotseat.getScaleX();
        		mOldHotseatScaleY = hotseat.getScaleY();
        		mOldHotseatAlpha = hotseat.getAlpha();
        		
        		mNewHotseatTranslationY = translationY;
        		if(stateIsSpringLoaded){
        			mNewHotseatScaleX = 1.0f;	
        			mNewHotseatScaleY = 1.0f;	
        		}else{
        			mNewHotseatScaleX = finalScaleFactor;
        			mNewHotseatScaleY = finalScaleFactor;
        		}
        		if(stateIsNormal || stateIsSpringLoaded){
        			mNewHotseatAlpha = 1.0f;
        		}else{
        			mNewHotseatAlpha = 1.0f-mOldHotseatAlpha;
        		}
        		
        	}else{
        		hotseat.setTranslationY(translationY);
        		if(stateIsSpringLoaded){
        ...
        */
        //caoshiyu add for dot indicator
        final DotIndicator dotIndicator = getDotIndicatorView();
        
        if(dotIndicator != null){
        	if(animated){
        		mOldDotIndTranslationY = dotIndicator.getTranslationY();
        		mOldDotIndScaleX = dotIndicator.getScaleX();
        		mOldDotIndScaleY = dotIndicator.getScaleY();
        		mOldDotIndAlpha = dotIndicator.getAlpha();
        		if(stateIsZoom && oldStateIsNormal){
        			mNewDotIndTranslationY = -mDotIndTranslateY;
        		}else if(stateIsNormal && oldStateIsZoom){
        			mNewDotIndTranslationY = 0;
        		}else{
        			mNewDotIndTranslationY = translationY;
        		}
        		if(stateIsSpringLoaded){
        			mNewDotIndScaleX = 1.0f;	
        			mNewDotIndScaleY = 1.0f;	
        		}else{
        			mNewDotIndScaleX = finalScaleFactor;
        			mNewDotIndScaleY = finalScaleFactor;
        		}
        		if(stateIsNormal || stateIsSpringLoaded || stateIsZoom){
        			mNewDotIndAlpha = 1.0f;
        		}else{
        			mNewDotIndAlpha = 1.0f-mOldDotIndAlpha;
        		}
        		
        	}else{
        		if(stateIsZoom && oldStateIsNormal){
        			dotIndicator.setTranslationY(-mDotIndTranslateY);
        		}else{
        			dotIndicator.setTranslationY(translationY);
        		}
        		if(stateIsSpringLoaded){
        			dotIndicator.setScaleX(1.0f);
        			dotIndicator.setScaleY(1.0f);
        		}else{
        			dotIndicator.setScaleX(finalScaleFactor);
        			dotIndicator.setScaleY(finalScaleFactor);
        		}
           		if(stateIsNormal || stateIsSpringLoaded || stateIsZoom){
           			dotIndicator.setAlpha(1.0f);
        		}else{
        			dotIndicator.setAlpha(0.0f);
        		}
        	}
        }
        if (animated) {
            ...
            animWithInterpolator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(android.animation.Animator animation) {
                    ...
                    if (!mFadeInAdjacentScreens && stateIsNormal && (oldStateIsSmall||oldStateIsZoom)) {
                        for (int i = 0; i < getChildCount(); i++) {
                            final CellLayout cl = (CellLayout) getChildAt(i);
                            cl.setAlpha(1f);
                        }
                        if(dotIndicator != null){
                        	dotIndicator.setTranslationY(0.0f);
                        	dotIndicator.setScaleX(1.0f);
                        	dotIndicator.setScaleY(1.0f);
                        	dotIndicator.setAlpha(1.0f);
//                        	dotIndicator.invalidate();
                    ...
                    if(stateIsZoom && oldStateIsNormal){
                        if(dotIndicator != null){
                        	dotIndicator.setTranslationY(mNewDotIndTranslationY);
                        	dotIndicator.setScaleX(mNewDotIndScaleX);
                        	dotIndicator.setScaleY(mNewDotIndScaleY);
                        	dotIndicator.setAlpha(mNewDotIndAlpha);
                        }
                    }
                }
				@Override
...
//                        cl.setFastScaleY(a * mOldScaleYs[i] + b * mNewScaleYs[i]);
                        Log.d("caoshiyu-changeStateAnimation", "mNewScaleXs---->"+mNewScaleXs[i]+",mNewScaleYs--->"+mNewScaleYs[i]);
...
//                        cl.setAlpha(a * mOldAlphas[i] + b * mNewAlphas[i]);
                    ...
                    syncChildrenLayersEnabledOnVisiblePages();
                    //add for dot indicator
                    if(dotIndicator != null){
                    	dotIndicator.setTranslationY(a * mOldDotIndTranslationY + b * mNewDotIndTranslationY);
                    	dotIndicator.setScaleX(a * mOldDotIndScaleX + b * mNewDotIndScaleX);
                    	dotIndicator.setScaleY(a * mOldDotIndScaleY + b * mNewDotIndScaleY);
                    	dotIndicator.setAlpha(a * mOldDotIndAlpha + b * mNewDotIndAlpha);
                    }
                }
            });
...
//        stopDragAppWidget(mCurrentPage);
    ...
    ArrayList<CellLayout> getWorkspaceAndHotseatCellLayouts() {
        ...
        if (mLauncher.getHotseat() != null && hasHotseat()) {
                ...
                public void run() {
                    ...
                    for (int j = 0; j < childCount; j++) {
                        ...
                        if (tag instanceof PreShortcutInfo) {
                        	Log.i(TAG, "removeItems tag = "+tag.toString());
	...
	private ArrayList<Integer> mUsedPages = null;
	private ArrayList<ArrayList<View>> mPageItemTemp = null;
	private ArrayList<Integer> mWaitReleaseWidgets = null;

	void initScreens(int screen_num){
    	CellLayout screen;

    	LayoutInflater inflater = mLauncher.getLayoutInflater();
    	if(screen_num == getChildCount())
    	{
    	}
    	else if(screen_num <= 0 || screen_num >mLauncher.getScreenCountMax())
    	{
       	}
    	else if(screen_num > getChildCount())
    	{
    		for(int i = getChildCount(); i < screen_num; i ++)
    		{
    			screen = (CellLayout)inflater.inflate(R.layout.workspace_screen, Workspace.this, false);
    	...
    	else if(screen_num < getChildCount())
    	{
    		...
    		for(int i=screen_num;i < childNum; i++){
    			removeViewAt(screen_num);
    		}
    	}
    }
	
	private void saveItem(View itemView, ItemInfo itemInfo, int page_index){
		CellLayout.LayoutParams lp = (CellLayout.LayoutParams) itemView.getLayoutParams();
        LauncherModel.moveItemInDatabase(mLauncher, itemInfo,
                LauncherSettings.Favorites.CONTAINER_DESKTOP, page_index, lp.cellX, lp.cellY);
    }
	
	private void deleteItemUtilSecondStep(){
        final LauncherAppWidgetHost appWidgetHost = mLauncher.getAppWidgetHost();
		new Thread("deleteAppWidgetId") {
			public void run() {
				for(int id:mWaitReleaseWidgets){
					appWidgetHost.deleteAppWidgetId(id);
				}
				mWaitReleaseWidgets.clear();
				mWaitReleaseWidgets = null;
			}
		}.start();
	}
	
	private void deleteItemUtil(ItemInfo item){
        Log.v("WorkspacePreviewer", "deleteItemUtil(): " + item);
        ...
        if (item instanceof ShortcutInfo) {
            ...
            if (appWidgetHost != null) {
            	mWaitReleaseWidgets.add(launcherAppWidgetInfo.appWidgetId);
            }
        }
	}

	private void getPageItems(CellLayout page, ArrayList<View> itemsArray){
		itemsArray.clear();

		for(int i=0; i<page.getChildrenChildCount();i++){
			itemsArray.add(page.getChildrenChildAt(i));
		}
	}

	private void removePageItems(int page){
		CellLayout cell = (CellLayout)getChildAt(page);
		cell.removeAllViews();
	}
	
	private void addPageItems(int page, ArrayList<View> itemsArray){
		CellLayout cell = (CellLayout)getChildAt(page);
		for(View item:itemsArray){
	        ...
	        int childId = LauncherModel.getCellLayoutChildId(LauncherSettings.Favorites.CONTAINER_DESKTOP, 
	        		page, itemInfo.cellX, itemInfo.cellY, itemInfo.spanX, itemInfo.spanY);
	        boolean markCellsAsOccupied = !(item instanceof Folder);
	        cell.addViewToCellLayout(item, -1, childId, lp, markCellsAsOccupied); 
			
			saveItem(item, itemInfo, page);
		}
	}

	public void adjustAllPages(ArrayList<Integer> pageOrder) {
		if (mUsedPages == null) {
			mUsedPages = new ArrayList<Integer>();
		}
		mUsedPages.clear();
		if (mPageItemTemp == null) {
			mPageItemTemp = new ArrayList<ArrayList<View>>();
		}
		mPageItemTemp.clear();
		if (mWaitReleaseWidgets == null) {
			mWaitReleaseWidgets = new ArrayList<Integer>();
		}

		int oldPageCount = getChildCount();
		Log.v("WorkspacePreviewer", "old page count:" + oldPageCount);
		boolean removedScreenRec[] = new boolean[oldPageCount]; 

		/* fetch all items to mPageItemTemp */
		for (int i = 0; i < oldPageCount; i++) {
			CellLayout page = (CellLayout) getChildAt(i);
			ArrayList<View> pageItems = new ArrayList<View>();
			getPageItems(page, pageItems);
			mPageItemTemp.add(pageItems);
			removedScreenRec[i] = false;
		}

		/* create new pages if need */
		if (pageOrder.size() > oldPageCount) {
			int newPagesNum = pageOrder.size() - oldPageCount;
			LayoutInflater inflater = mLauncher.getLayoutInflater();
			Log.v("WorkspacePreviewer", "new pages:" + newPagesNum);
			for (int i = 0; i < newPagesNum; i++) {
				CellLayout emptyPage = (CellLayout) inflater.inflate(
						R.layout.workspace_screen, Workspace.this, false);
				emptyPage.setFocusable(true);
				emptyPage.setOnLongClickListener(mLauncher);
				addView(emptyPage);
//				addDots();
			}
		}
		...
		for (int i = 0; i < pageOrder.size(); i++) {
			int order = (int) pageOrder.get(i);
			Log.v("WorkspacePreviewer", "adjustAllPages(), order:" + order);
			if (order >= mLauncher.getScreenCountMax()) {
				// a new page, means this is a empty page
				Log.v("WorkspacePreviewer", "adjustAllPages(), new page:"
						+ order);
			} else if (order >= 0 && order < oldPageCount) {
				int from = order;
				int to = i;
				mUsedPages.add(from);
				if (from == to)
				...
				if (!removedScreenRec[from]) {
					removePageItems(from);
					removedScreenRec[from] = true;
				}
				// remove to page items
				if (!removedScreenRec[to]) {
					removePageItems(to);
					removedScreenRec[to] = true;
				}
				// add to page from mPageItemTemp
				Log.v("WorkspacePreviewer", "move items from page" + from
						+ " to:" + to);
				addPageItems(to, mPageItemTemp.get(from));
			}
		}

		for (int j = 0; j < mUsedPages.size(); j++) { // if store in mUsedPages
			int page = mUsedPages.get(j);
			Log.v("WorkspacePreviewer", "used page:" + page);
		}
		// make sure delete pages
		for (int i = 0; i < oldPageCount; i++) { // ervery page
			boolean find = false;
			for (int j = 0; j < mUsedPages.size(); j++) { // if store in
															// mUsedPages
				int page = mUsedPages.get(j);
				if (page == i) {
					find = true;
			...
			// delete items in page
			if (!find) {
				Log.v("WorkspacePreviewer", "delete page:" + i);
				if (!removedScreenRec[i]) {
					removePageItems(i);
					removedScreenRec[i] = true;
				}
				for (View view : mPageItemTemp.get(i)) {
					ItemInfo itemInfo = (ItemInfo) view.getTag();
					deleteItemUtil(itemInfo);
				}
			}
		}
		deleteItemUtilSecondStep();

		// adjust mCurrentScreen and remove screen if need
		int page_num = pageOrder.size();
		if (mCurrentPage >= page_num)
			mCurrentPage = page_num - 1;
		mLauncher.setScreenCount(page_num);
		resetDefaultScreen();
		if (page_num < getChildCount()) {
			removeViews(page_num, getChildCount() - page_num);
//			removeDots();
		}
		mTouchState = TOUCH_STATE_REST;
		mNextPage = INVALID_PAGE;
		Log.v("WorkspacePreviewer", "adjustAllPages(), mTouchState:"
				+ mTouchState + " mNextScreen:" + mNextPage);
		mUsedPages.clear();
		mPageItemTemp.clear();
	}
	
	public int resetDefaultScreen() {
		final int screenCount = mLauncher.getScreenCount();
		final int defScreen = (screenCount - 1) / 2;
		mDefaultPage = defScreen;
		mLauncher.setScreen(mDefaultPage);
		return mDefaultPage;
	}

	public void initCurrentScreenIndex() {
		mCurrentPage = mDefaultPage;
	}
