...
import com.hct.scene.launcher2.LauncherSettings.Favorites;
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String AUTHORITY = "com.hct.scene.launcher2.settings";

    static final String TABLE_FAVORITES = "favorites";
    static final String PARAMETER_NOTIFY = "notify";
    
    static boolean mLoadFavorites = false;
    /**
    ...
    static public boolean getLoadFavorites(){
    	return mLoadFavorites;
    }
    static public void setLoadFavorites(boolean flag){
    	mLoadFavorites = flag;
    }
    static class DatabaseHelper extends SQLiteOpenHelper {
    	public static final int[] PRESCENES = new int[] {
            R.xml.default_workspace, R.xml.work, R.xml.house
    	};
...
//			R.xml.default_workspace
        ...
        private static final String TAG_PRESHORTCUT = "preshortcut";        
        private static final String TAG_FURNITURE = "furniture";
        private static final String TAG_BOOKSHELF_ITEM = "bookshelf";

        private final Context mContext;
        private final AppWidgetHost mAppWidgetHost = null;
...
//            }
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            final int furnitureClassIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.FURNITURE_CLASS);
            final int furnitureStyleIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.FURNITURE_STYLE);
            ...
            while (c.moveToNext()) {
                ...
                values.put(LauncherSettings.Favorites.FURNITURE_CLASS, c.getString(furnitureClassIndex));
                values.put(LauncherSettings.Favorites.FURNITURE_STYLE, c.getInt(furnitureStyleIndex));
                rows[i++] = values;
            }
...
//                loadScene(db, R.xml.default_workspace);
            ...
            try {
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        added = id >= 0;
                    }else if (TAG_PRESHORTCUT.equals(name)){
                    	added = addPreShortcut(db, values, a, packageManager, intent);
                    ...
                    }else if(TAG_FURNITURE.equals(name)){
                    	 long id = addFurniture(db, values, a);
                         added = id >= 0;
                    } else if (TAG_BOOKSHELF_ITEM.equals(name)) {
                    	long id = addAppShortcut(db, values, a, packageManager, intent);
                        added = id >= 0;
                    }
        ...
        private long addFurniture(SQLiteDatabase db, ContentValues values, TypedArray a){
            long id = -1;
            String className = a.getString(R.styleable.Favorite_furnitureClassName);
            String style = a.getString(R.styleable.Favorite_furnitureStyle); 
            ...
            id = generateNewId();
            values.put(Favorites.FURNITURE_CLASS, className);
            values.put(Favorites.FURNITURE_STYLE, style);
            values.put(Favorites.TITLE, title);
            values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_FURNITURE);
        ...
        private boolean addPreShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
                PackageManager packageManager, Intent intent) {
            ...
            Log.i(TAG, "addPreShortcut--packageName="+packageName);
            intent.setComponent(cn);
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
                    Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
            values.put(Favorites.INTENT, intent.toUri(0));
//                values.put(Favorites.TITLE, info.loadLabel(packageManager).toString());
            values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_PRE_SHORTCUT);
            values.put(Favorites.SPANX, 1);
            values.put(Favorites.SPANY, 1);
