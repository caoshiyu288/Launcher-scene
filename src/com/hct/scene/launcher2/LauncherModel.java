...
public class LauncherModel extends BroadcastReceiver {
    ...
    static final ArrayList<ItemInfo> sHouseWorkspaceItems = new ArrayList<ItemInfo>();

    public static final ArrayList<ItemInfo> sSceneBookshelfItems = new ArrayList<ItemInfo>();  /* added by hct lijunyi, 20130130 */
    ...
    private static int mHouseCellCountX;
    private static int mHouseCellCountY;

    protected int mPreviousConfigMcc;
    
    protected String mPreviousConfigSkin;       //add by hct chenxin
    ...
    static final String SYSTEM_SCENE_HOUSE = "house";

    static final String[] SYSTEM_SCENE_NAMES = {
            SYSTEM_SCENE_DEFAULT, SYSTEM_SCENE_WORK, SYSTEM_SCENE_HOUSE
    };
    ...
    public interface Callbacks {
        ...
        public void setThemeWallpaer(String name);//add by hct chenxin
    }
    ...
    LauncherModel(LauncherApplication app, IconCache iconCache) {
        ...
        mPreviousConfigSkin = config.skin;//add by hct chenxin
    }
    ...
    static void updateItemInDatabaseHelper(Context context, final ContentValues values,
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                }else if(modelItem.container == LauncherSettings.Favorites.CONTAINER_HOUSE){
                	if(!sHouseWorkspaceItems.contains(modelItem)){
                		sHouseWorkspaceItems.add(modelItem);
                	}
				} else if (modelItem.container == LauncherSettings.Favorites.CONTAINER_BOOKSHELF) {
					if (!sSceneBookshelfItems.contains(modelItem)) {
						sSceneBookshelfItems.add(modelItem);
					}
                } else {
    ...
    static void addItemToDatabase(Context context, final ItemInfo item, final long container,
            final int screen, final int cellX, final int cellY, final String scene, final boolean notify) {
        item.container = container;
        item.cellX = cellX;
        item.cellY = cellY;
        // We store hotseat items in canonical form which is this orientation invariant position
            ...
            public void run() {
                ...
                switch (item.itemType) {
                    ...
                    case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                        ...
                        if (item.container == LauncherSettings.Favorites.CONTAINER_BOOKSHELF) {
                        	sSceneBookshelfItems.add(item);
                        }
                        break;
    ...
    static void addItemToDatabase(Context context, final ItemInfo item, final long container,
    	...
    	addItemToDatabase(context, item, container, screen, cellX, cellY, Launcher.getCurrentScene(), notify);
    }
    /**
    ...
    static int getHouseCellCountX() {
    	return mHouseCellCountX;
    }
    
    static int getHouseCellCountY() {
    	return mHouseCellCountY;
    }
     ...
     * when performing local/canonical coordinate transformations.
     */
    static void updateHouseWorkspaceLayoutCells(int shortAxisCellCount, int longAxisCellCount) {
    	mHouseCellCountX = shortAxisCellCount;
    	mHouseCellCountY = longAxisCellCount;
    }
            ...
            public void run() {
                ...
                switch (item.itemType) {
                    ...
                    case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                        ...
                        if (item.container == LauncherSettings.Favorites.CONTAINER_BOOKSHELF) {
                            sSceneBookshelfItems.remove(item);  /* added by hct lijunyi, 20130205 */
                        }
                        break;
                    case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            forceReload();
             ...
             if(!mPreviousConfigSkin.equals(currentConfig.skin)){
            	 if (mCallbacks != null) {
             		Callbacks callbacks = mCallbacks.get();
             		if (callbacks != null) {
             			callbacks.setThemeWallpaer("theme_wallpaper");
             		}
             	}
             }
             mPreviousConfigMcc = currentConfig.mcc;
             mPreviousConfigSkin = currentConfig.skin; //add by hct chenxin
    ...
    private class LoaderTask implements Runnable {
        ...
        private void waitForIdle() {
            ...
            synchronized (LoaderTask.this) {
                ...
                mHandler.postIdle(new Runnable() {
                        public void run() {
                        	Log.d(TAG, "last runnable is running");
                            synchronized (LoaderTask.this) {
        ...
        private boolean checkItemPlacement(ItemInfo occupied[][][], ItemInfo item) {
            ...
            if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
				...
				/* modify by hct, 20121011, Launcher.SCREEN_COUNT -> Launcher.getScreenCount() */
                if (occupied[Launcher.getScreenCount()][item.screen][0] != null) {
                    Log.e(TAG, "Error loading shortcut into hotseat " + item
                        + " into position (" + item.screen + ":" + item.cellX + "," + item.cellY
                        + ") occupied by " + occupied[Launcher.getScreenCount()][item.screen][0]);
                    return false;
                } else {
                    occupied[Launcher.getScreenCount()][item.screen][0] = item;
                    return true;
                }
        ...
        private void loadWorkspace() {
            ...
            sItemsIdMap.clear();
            sDbIconCache.clear();
            sHouseWorkspaceItems.clear();
            sSceneBookshelfItems.clear();
            ...
            final ItemInfo occupied[][][] =
                    new ItemInfo[Launcher.getScreenCount() + 1][mCellCountX + 1][mCellCountY + 1];
            ...
            try {
                ...
                //caoshiyu add
                final int furnitureClassIndex = c.getColumnIndexOrThrow(
                		LauncherSettings.Favorites.FURNITURE_CLASS);
                final int furnitureStyleIndex = c.getColumnIndexOrThrow(
                		LauncherSettings.Favorites.FURNITURE_STYLE);
                ...
                while (!mStopped && c.moveToNext()) {
                    try {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_PRE_SHORTCUT:		
                            ...
                            if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                            ...
                            }else if (itemType == LauncherSettings.Favorites.ITEM_TYPE_PRE_SHORTCUT) {
                            	int pre_shortcut_x = c.getInt(cellXIndex);
                            	int pre_shortcut_y = c.getInt(cellYIndex);
                            	//intent.getComponent().getPackageName();
                    			String[] apk_name = mApp.getResources().getStringArray(R.array.pre_apk_filename) ;
                    			String[] apk_packname = mApp.getResources().getStringArray(R.array.pre_apk_packname) ;
                    			String preShortpgname = intent.getComponent().getPackageName();                            	
                            	...
                            	Log.e(TAG, "LauncherSettings pre_shortcut_id: " + pre_shortcut_x);
                            	info = getPreShortcutInfo(mindex,context);
                            	if(info != null)
									Log.i(TAG, "getPreShortcutInfo-info.icon="
											+ info.mIcon + ", title=="
											+ info.title + ", cellXIndex=="+cellXIndex);
							} else {
							    if(!context.getResources().getBoolean(R.bool.config_support_icon_background)){
                            ...
                            if (info != null) {
								...
								case LauncherSettings.Favorites.CONTAINER_BOOKSHELF: {
									sSceneBookshelfItems.add(info);
									break;
								}
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_FURNITURE:
                        	furnitureInfo = getFurnitureInfo(c, furnitureClassIndex, furnitureStyleIndex, 
                        			titleIndex);
                        	if(furnitureInfo != null){
                        		furnitureInfo.id = c.getLong(idIndex);
                        		furnitureInfo.screen = c.getInt(screenIndex);
                        		furnitureInfo.cellX = c.getInt(cellXIndex);
                        		furnitureInfo.cellY = c.getInt(cellYIndex);
                        		container = c.getInt(containerIndex);
                        		furnitureInfo.container = container;
                        		furnitureInfo.spanX = c.getInt(spanXIndex);
                        		furnitureInfo.spanY = c.getInt(spanYIndex);
                        		switch (container) {
                        		case LauncherSettings.Favorites.CONTAINER_HOUSE:
                        			sHouseWorkspaceItems.add(furnitureInfo);
                        			break;
                        		}
                        		sItemsIdMap.put(furnitureInfo.id, furnitureInfo);
                        	}
                        	break;
                        }
                    } catch (Exception e) {
        ...
        private void bindWorkspace() {
            ...
            if(Launcher.isHouseWorkspace()){
            	//add for furniture:
            	final ArrayList<ItemInfo> houseWorkspaceItems = new ArrayList<ItemInfo>(sHouseWorkspaceItems);
            	// Add the items to the house workspace.
            	N = houseWorkspaceItems.size();
            	for (int i=0; i<N; i+=ITEMS_CHUNK) {
            		...
            		mHandler.post(new Runnable() {
            			public void run() {
            				Callbacks callbacks = tryGetCallbacks(oldCallbacks);
            				if (callbacks != null) {
            					callbacks.bindItems(houseWorkspaceItems, start, start+chunkSize);
            				}
            			}
            		});
            	}
            	
            	final ArrayList<ItemInfo> sceneBookshelfItems = new ArrayList<ItemInfo>(sSceneBookshelfItems);
            	N = sceneBookshelfItems.size();
            	Bookstore.mSelectIndex = N;
            	for (int i=0; i<N; i+=ITEMS_CHUNK) {
            		final int start = i;
            		final int chunkSize = (i+ITEMS_CHUNK <= N) ? ITEMS_CHUNK : (N-i);
            		...
            		mHandler.post(new Runnable() {
            			public void run() {
            				...
            				if (callbacks != null) {
            					callbacks.bindItems(sceneBookshelfItems, start, start+chunkSize);
            				}
            			}
            		});
            	}
            ...
            if(Launcher.isDefaultWorkspace()){
            ...
            try {
                ...
                if (resources != null) {
                    ...
                    icon = Utilities.createIconBitmap(
                            mIconCache.getFullResIcon(resources, id), context);
            //add by hct-zhangcy
                    if(mIconCache.isPreinstallApp(intent.getComponent().getPackageName())
                       || mIconCache.isDownloadApp(intent.getComponent().getPackageName())){
		                ComponentName componentName = intent.getComponent();
		                Bitmap dst = mIconCache.getBgimage(componentName);
		                if(dst == null){
		                  dst = Utilities.createIconBitmap(
		                        context.getResources().getDrawable(mIconCache.generateRandomImg()), context);
		                  mIconCache.SetBgimage(componentName,dst);
		                }
                        icon = Utilities.createCompoundBitmap(dst,icon); 
                    } 
           //end
                }
            } catch (Exception e) {
...
//add by zhangcy-end
    private FurnitureInfo getFurnitureInfo(Cursor c, int furnitureClassIndex, 
    		int furnitureStyleIndex, int titleIndex){
    	Resources resources = mApp.getResources();
    	final FurnitureInfo fntInfo = new FurnitureInfo();
    	fntInfo.title = c.getString(titleIndex);
    	final String wholeClassName = c.getString(furnitureClassIndex);
    	fntInfo.className = wholeClassName;
    	final int style = c.getInt(furnitureStyleIndex);
    	fntInfo.style = style;
    	/*
    	Drawable bg = null;
    	final String packageName = resources.getResourcePackageName(R.array.furniture_style);

    	final String simpleClassName = getFurnitureSimpleClassName(wholeClassName);
    	Log.d("furniture", "getFurnitureInfo-packageName=="+packageName+"," +
    			"furnitureShortClassName=="+simpleClassName);
    	final int furnitureArrayId = resources.getIdentifier(simpleClassName, "array", packageName);
    	String[] res = resources.getStringArray(furnitureArrayId);
    	if(res.length < 2){
    		throw new NotFoundException("furniture class bg is not enough!!!");
    	} 
    	bg = resources.getDrawable(resources.getIdentifier(res[style], "drawable", packageName));
    	if(bg != null){
    		fntInfo.setBg(bg);
    	}
    	*/
    ...
    /**
     * Make an Preset ShortcutInfo object for a preshortcut that isn't an application.
     */
    private ShortcutInfo getPreShortcutInfo(int index, Context context){
        Bitmap icon = null;
        int mindex = 0 ; 
                
        final Resources resources = mApp.getResources();
        final String packageName = resources.getResourcePackageName(R.array.pre_shortcut_info);
        final PreShortcutInfo info = new PreShortcutInfo();
        String[] pre_res = resources.getStringArray(R.array.pre_shortcut_info) ;
        
        mindex = index ;
    	int icon_res = resources.getIdentifier(pre_res[mindex], "drawable", packageName);
    	int str_res = resources.getIdentifier(pre_res[mindex], "string", packageName);
    	    	
    	icon = Utilities.createIconBitmap(resources.getDrawable(icon_res), context);
    	String title = resources.getString(str_res); 
    ...
    ShortcutInfo addShortcut(Context context, Intent data, long container, int screen,
	  ...
	  if(context.getResources().getBoolean(R.bool.config_support_icon_background)){ 
         if(ACTION_INSTALL_SHORTCUT.equals(data.getAction())
           && (mIconCache.isPreinstallApp(info.intent.getComponent().getPackageName())
           || mIconCache.isDownloadApp(info.intent.getComponent().getPackageName()))){
            bgBitmap = mIconCache.getBgimage(info.intent.getComponent());
             if(bgBitmap != null){
               iconBitmap = info.getIcon(mIconCache);
               iconBitmap = Utilities.createCompoundBitmap(bgBitmap, iconBitmap);
               info.setIcon(iconBitmap);
             }
		  }
        }
//        addItemToDatabase(context, info, container, screen, cellX, cellY, notify);
        addItemToDatabase(context, info, container, screen, cellX, cellY, "default", notify);
    ...
    public static boolean hasLoadFavorites(Context context){
    	LauncherApplication app = (LauncherApplication)context.getApplicationContext();
    	if(app.getLauncherProvider().getLoadFavorites()){
    		return true;
    	}
    	return false;
    }
    public static void setLoadFavorites(Context context, boolean flag){
    	LauncherApplication app = (LauncherApplication)context.getApplicationContext();
    	app.getLauncherProvider().setLoadFavorites(flag);
    }
	public static Furniture createFurniture(String classname, Context context){
		Furniture furniture = null;
		try {
			Class newClass = Class.forName(classname);
			Constructor cons = newClass.getConstructor(Context.class);
//			runDb = (RunDb) cons.newInstance();
//			furniture = (Furniture) newClass.newInstance();
			furniture = (Furniture) cons.newInstance(context);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static String getFurnitureSimpleClassName(String wholeName){
		String name = null;
    	int dot = wholeName.lastIndexOf('.');
        if (dot != -1) {
            return wholeName.substring(dot + 1);
        }
		return null;
	}
}
