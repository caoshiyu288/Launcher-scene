...
import com.hct.scene.launcher2.DropTarget.DragObject;
...
public final class Launcher extends Activity
    ...
    private static final int MENU_GROUP_SCENE_CLASSIC = 1;
    private static final int MENU_GROUP_WALLPAPER = MENU_GROUP_SCENE_CLASSIC + 1;
    private static final int MENU_GROUP_SCENE_HOUSE = MENU_GROUP_WALLPAPER+1;
    private static final int MENU_SCENE_CLASSIC = Menu.FIRST + 1;
    private static final int MENU_WALLPAPER_SETTINGS = MENU_SCENE_CLASSIC + 1;
    private static final int MENU_MANAGE_APPS = MENU_WALLPAPER_SETTINGS + 1;
    private static final int MENU_SYSTEM_SETTINGS = MENU_MANAGE_APPS + 1;
    private static final int MENU_SCENE_HOUSE = MENU_SYSTEM_SETTINGS + 1;
    private static final int MENU_HELP = MENU_SCENE_HOUSE + 1;
	...
	/* added by hct, 20121011 */
	private ScreenPreview mScreenPreview;

    static final int WORKSPACE_DEFAULT_SCREEN_COUNT = 5;
    static final int WORKSPACE_MAX_PAGE_NUM = 7;
    static final int WORKSPACE_MIN_PAGE_NUM = 3;

	static final String WS_SCREEN_BASE = "Workspace_scr_base";
	static final String KEY_WS_SCREEN_NUM = "workspace_scr_nun";

    private static int SCREEN_COUNT = WORKSPACE_DEFAULT_SCREEN_COUNT;  // modify to private by zhouzhigang
    private static int DEFAULT_SCREEN = WORKSPACE_DEFAULT_SCREEN_COUNT/2; // modify to private by zhouzhigang
	/* end of added by hct, 20121011 */
    ...
    private static final String HOUSE_WALL = "house_wall";

    /** The different states that Launcher can be in. */
    private enum State { WORKSPACE, APPS_CUSTOMIZE, APPS_CUSTOMIZE_SPRING_LOADED, HOUSEWORKSPACE};
    private State mState = State.WORKSPACE;
    private AnimatorSet mStateAnimation;
    private AnimatorSet mDividerAnimator;
    private AnimatorSet mHouseStyleTransferAnimator;
    ...
    private HouseStyleTransfer mHouseStyleTransfer;
    private Furniture mPetFurniture;
    ...
    private OnlyAppsPagedViewFrame mOnlyAppsPane;
    private OnlyAppsPagedView mOnlyAppsContent;
    ...
    private boolean mHouseWorkspaceLoading = true;
    ...
    private BooksSelector mBooksSelector;  /* added by hct lijunyi, 20130130 */
    ...
    private Runnable mBuildLayersRunnable = new Runnable() {
        public void run() {
            if(true){
            	...
            	if(mHouseWorkspace != null){
            		mHouseWorkspace.buildPageHardwareLayers();
            	}
            }
        }
    };
    ...
    */
    ...
    public final static String HOUSE_SCENE = "house";
    private boolean mSwitchScene;
    static final HashMap<String, String> sSceneWallpaper = new HashMap<String, String>();
    
    //caoshiyu add for mark the onstart status
    private boolean mLauncherStart; 

    /*chengzhi: add the preshortcut 20121128*/
    private String mApkList[]=null;
    private static final String mapkDir = "/presetapks";      
    
    public static final int DEFAULT_HOUSE_SCREEN = 1;
    public static final int MAX_HOUSE_SCREEN = 8;
    
    /*liuyang add for door @20130130*/
    private Door mDoor;
    
    public static int mWorkspaceEffect = -1;
    public static int mAppsEffect = -1;
    int mOriginalWorkspaceEffectStyle = -1;
    int mOriginalAppsEffectStyle = -1;
    private ArrayList<ResolveInfo> mHomeLauncherResolveInfos = new ArrayList<ResolveInfo>();
	private ComponentName mOtherLauncherComponent = null;
	private static final String KILL_EXCESS_LAUNCHER_ACTION = "com.hct.launcher.action.KILL_EXCESS_LAUNCHER";
	private static final String KILL_EXCESS_LAUNCHER_PERMISSION = "com.hct.launcher.permission.KILL_EXCESS_LAUNCHER";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        mCurrentScene = getSharedPreferences(CURRENT_SCENE, 0).getString(CURRENT_SCENE, "house");
        ...
        /*hct:remove widgets in all apps
...
//        startService(new Intent(this, WeatherProviderService.class));
        
        if(getIntent().getBooleanExtra("hct_launcher", false) && savedInstanceState == null){
        	getIntent().putExtra("hct_launcher", false);
        	mOtherLauncherComponent = getIntent().getParcelableExtra("hct_launcher_component");
        	Log.d("caoshiyu-door", "hct_launcher is true->"+mOtherLauncherComponent.getPackageName());
        	openDoor();
//        	Intent intent = new Intent(KILL_EXCESS_LAUNCHER_ACTION);
//        	sendBroadcast(intent);
        }
//        manageKillExcessLauncherReceiver();
    }
    private BroadcastReceiver mStopShelfReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(KILL_EXCESS_LAUNCHER_ACTION)) {
				try {
					Log.d("caoshiyu-door", "receiver-->" + this);
					Launcher.this.unregisterReceiver(mStopShelfReceiver);
					if (true) {
						((ActivityManager) Launcher.this.getSystemService("activity"))
								.forceStopPackage(getPackageName());
					}
				} catch (Exception e) {
					Log.d("caoshiyu-door", "error!!!!" + this);
					e.printStackTrace();
				}
			}
		}
	};
    private void manageKillExcessLauncherReceiver(){
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(KILL_EXCESS_LAUNCHER_ACTION);
        registerReceiver(mStopShelfReceiver, intentFilter, KILL_EXCESS_LAUNCHER_PERMISSION, null);
    }
    private void checkForLocaleChange() {
        ...
        //add by hct chenxin
        final String previousSkin = sLocaleConfiguration.skin;
        final String skin = configuration.skin;
        Log.v("checkForLocaleChange", "previousSkin = " +previousSkin);
        Log.v("checkForLocaleChange", "skin = " +skin);
        boolean localeChanged = !skin.equals(previousSkin) || !locale.equals(previousLocale) || mcc != previousMcc || mnc != previousMnc;
        ...
        if (localeChanged) {
            ...
            sLocaleConfiguration.skin = skin; //add by hct chenxin
    ...
    private static class LocaleConfiguration {
        ...
        public int mcc = -1;
        public int mnc = -1;
        public String skin;  //add by hct chenxin
    }
    ...
    private static void readConfiguration(Context context, LocaleConfiguration configuration) {
        ...
        try {
            ...
            configuration.skin = in.readUTF(); //add by hct chenxin
    ...
    private static void writeConfiguration(Context context, LocaleConfiguration configuration) {
        ...
        try {
            ...
            out.writeUTF(configuration.skin);//add by hct chenxin
            out.flush();
        } catch (FileNotFoundException e) {
            // Ignore
        } catch (IOException e) {
...
//        getWindow().getDecorView().getViewTreeObserver().addOnPostDrawListener(mPostDrawListener);
        ...
        if (isAllAppsVisible() == true && mAppsCustomizeTabHost != null && 
        ...
        //caoshiyu add
        if(isAllAppsVisible() && mOnlyAppsPane.getVisibility() == View.VISIBLE &&
        		mOnlyAppsPane.getContentVisibility() == View.GONE){
        	mOnlyAppsPane.setContentVisibility(View.VISIBLE);
        }
    }
    ...
    protected void onStop() {
        ...
        if(isHouseWorkspace()&&mHouseWorkspace.getHouseIconsContainer() != null
				&&mHouseWorkspace.getHouseIconsContainer().isAnimateOn())
        	mHouseWorkspace.getHouseIconsContainer().startShakeAnimate(false);
    	// Launch performance
//        getWindow().getDecorView().getViewTreeObserver().removeOnPostDrawListener(mPostDrawListener);
    ...
    protected void onResume() {
        ...
        //if(isDefaultWorkspace()){
        	if(mWorkspace != null){
        		if(mOriginalWorkspaceEffectStyle == -1)
        			mWorkspaceEffect = mWorkspace.getEffectStyle(Workspace.WORKSPACE_EFFECT_KEY,
        					getResources().getInteger(R.integer.config_default_workspace_multi_effect));
        	}
        	if(mAppsCustomizeContent != null){
        		if(mOriginalAppsEffectStyle == -1)
        			mAppsEffect = mAppsCustomizeContent.getEffectStyle(AppsCustomizePagedView.APPS_EFFECT_KEY,
        					getResources().getInteger(R.integer.config_default_apps_multi_effect));
        	}
        //}
        ...
        if(!mWorkspaceLoading){
//        	if(!mHouseWorkspaceLoading){
            final ViewTreeObserver observer = mHouseWorkspace.getViewTreeObserver();
            final HouseWorkspace workspace = mHouseWorkspace;
            // We want to let Launcher draw itself at least once before we force it to build
            // layers on all the workspace pages, so that transitioning to Launcher from other
            // apps is nice and speedy. Usually the first call to preDraw doesn't correspond to
            // a true draw so we wait until the second preDraw call to be safe
...
//		mWorkspace.onPauseWhenShown(mWorkspace.getCurrentPage());
		...
		if (mWorkspace != null && isDefaultWorkspace()) {
			mWorkspace.setCurrentPage(mWorkspace.getCurrentPage());
		}else if(mHouseWorkspace != null && isHouseWorkspace()){
			mHouseWorkspace.setCurrentPage(mHouseWorkspace.getCurrentPage());
		}
        mPaused = true;
    ...
    */
    ...
    private void restoreState(Bundle savedState) {
        ...
        if (state == State.APPS_CUSTOMIZE) {
        	if(isDefaultWorkspace()){
        		showAllApps(false);
        	}else if(isHouseWorkspace()){
        		showOnlyAllApps(false);
        	}
        }
    ...
    private void setupViews() {
        ...
        //caoshiyu add for house workspace
        mHouseWorkspace = (HouseWorkspace) mDragLayer.findViewById(R.id.house_workspace);
        mHouseWorkspace.setWall(getHouseWall());
        mHouseWorkspace.setOnLongClickListener(this);
        mHouseWorkspace.setOnClickListener(this);
        mHouseWorkspace.setup(dragController);
        mHouseWorkspace.initCurrentScreenIndex();
        mHouseStyleTransfer = (HouseStyleTransfer) mDragLayer.findViewById(R.id.house_style_transfer);
        mHouseStyleTransfer.setup(this);
        
        mOnlyAppsPane = (OnlyAppsPagedViewFrame) mDragLayer.findViewById(R.id.only_apps_pane);
        mOnlyAppsContent = (OnlyAppsPagedView)mOnlyAppsPane.findViewById(R.id.only_apps_content);
        mOnlyAppsContent.setup(this, dragController);
        mOnlyAppsContent.setDragLayer(mDragLayer);
        //pet
        mPetFurniture = (Furniture) mDragLayer.findViewById(R.id.pet);
        mPetFurniture.setSingleClickListener(this);
        //caoshiyu remove it
		...
		/* added by hct, 20121011 */
		final int screen_num = getSharedPreferences(WS_SCREEN_BASE, 0).getInt(KEY_WS_SCREEN_NUM, WORKSPACE_DEFAULT_SCREEN_COUNT);
		SCREEN_COUNT = screen_num;
		bindScreens(screen_num);
		setScreen(mWorkspace.resetDefaultScreen());
		mWorkspace.initCurrentScreenIndex();
		/* end of added by hct, 20121011 */
        ...
        // Setup AppsCustomize
        /* hct:remove the all apps in house launcher
        mAppsCustomizeTabHost = (AppsCustomizeTabHost)
                findViewById(R.id.apps_customize_pane);
        mAppsCustomizeContent = (AppsCustomizePagedView)
                mAppsCustomizeTabHost.findViewById(R.id.apps_customize_pane_content);
        mAppsCustomizeContent.setup(this, dragController);
        mAppsCustomizeContent.setDragLayer(mDragLayer);
        */
        ...
        /* added by hct, 20121011 */
		mScreenPreview = (ScreenPreview) findViewById(R.id.screen_thumb);
		if (null != mScreenPreview) {
			mScreenPreview.setLauncher(this);
		}
		/* end of added by hct, 20121011 */

        /*liuyang add for door @20130130*/
        mDoor = (Door) findViewById(R.id.door);
        mDoor.setCloseDoorCallback(mExitHouseRunnable);
        mDoor.setCompleteCallback(mCompleteDoorRunnable);
		/* added by hct lijunyi, 20130129, test code */
		mBooksSelector = (BooksSelector) findViewById(R.id.bookshelf_selector);
		mBooksSelector.setLauncher(this);
		hideBookSelector();
		...
		//init effect sytle
		initWorkspaceAndAppsEffectStyle();
    }
    
    void initWorkspaceAndAppsEffectStyle(){
    	mOriginalWorkspaceEffectStyle = getResources().getInteger(R.integer.config_enable_workspace_effect);
    	mOriginalAppsEffectStyle = getResources().getInteger(R.integer.config_enable_apps_effect);
    	if(mOriginalWorkspaceEffectStyle != -1){
    		mWorkspaceEffect = mOriginalWorkspaceEffectStyle;
    	}
    	if(mOriginalAppsEffectStyle != -1){
    		mAppsEffect = mOriginalAppsEffectStyle;
    	}
    }
	/* added by hct lijunyi, 20130130 */
	public void hideBookSelector() {
		if (null != mBooksSelector) {
			if (mBooksSelector.getVisibility() == View.VISIBLE) {
				if(!mSwitchScene){
					showPet();
				}
				mBooksSelector.hideLayout();	
				if(mAddAppsToHouseWorkspace != null){
					mAddAppsToHouseWorkspace.setInShowcase(false);
				}
			}
		}
	}

	public void showBookSelector(View view) {
		if (null != mBooksSelector) {
			if(mHouseWorkspace.getHouseIconsContainer().isAnimateOn())
				return;
			hidePet();
			mBooksSelector.showLayout(view);
	        mUserPresent = false;
	        updateRunning();
	...
	/* end of added by hct lijunyi, 20130130 */
    ...
    private void initWorkspace(){
    	if(isDefaultWorkspace()){
    		...
    		mState = State.HOUSEWORKSPACE;
    		mHouseWorkspace.setVisibility(View.VISIBLE);
    	}
    }
    /**
    ...
    View createFurniture(FurnitureInfo info){
    	Furniture furniture = LauncherModel.createFurniture(info.className.toString(), this);
    	furniture.setSingleClickListener(mHouseWorkspace);
    	furniture.setLongClickListener(this);
    	furniture.setAddAppsRunnable(mAddAppsToHouseWorkspace);
    	furniture.applyFromInfo(info);
    	if(furniture instanceof LoadHouseWorkspaceIcons){
    		mHouseWorkspace.setHouseIconsContainer((LoadHouseWorkspaceIcons)furniture);
    	}
//    	furniture.setOnClickListener(mHouseWorkspace);
//    	furniture.setOnLongClickListener(mHouseWorkspace);
    	return furniture;
    }
    
    View createHouseShortcut(ShortcutInfo info){
//		info.itemType = LauncherSettings.Favorites.ITEM_TYPE_APPLICATION;
		SceneShelfItem icon = (SceneShelfItem) mInflater.inflate(R.layout.scene_shelf_item, null);
//		TextView item = (TextView) view.findViewById(R.id.item);
//
//		Bitmap icon = Bitmap.createScaledBitmap(info.mIcon, mImageWidth, mImageHeight, false);
//
//		item.setText(info.title);
//		item.setCompoundDrawablesWithIntrinsicBounds(null, new BitmapDrawable(icon), null, null);
//
//		view.setTag(info);
		icon.applyFromShortcutInfo(info, mIconCache);
//		icon.setBackgroundResource(R.drawable.bookshelf_background_item);
    ...
    private void updateRunning() {
        ...
        if (autoAdvanceRunning != mAutoAdvanceRunning) {
        	Log.d("Launcher.Model", "updateRunning~~~~~~~~~~~~~~~~~~");
    ...
    protected void onNewIntent(Intent intent) {
        ...
        //open door
        if(intent.getBooleanExtra("hct_launcher", false)){
        	intent.putExtra("hct_launcher", false);
        	mOtherLauncherComponent = intent.getParcelableExtra("hct_launcher_component");
        	openDoor();
        }
        // Close the menu
...
//            	mWorkspace.moveOutAppWidget(mWorkspace.getCurrentPage());
            ...
            //caoshiyu add
            if(isHouseWorkspace()&&isAllAppsVisible()&&mOnlyAppsContent.isShake()){
            	mOnlyAppsContent.startShakeAnimate(false);
            	return;
            }
            if (isHouseWorkspace() && mHouseWorkspace.getHouseIconsContainer() != null
            		&& mHouseWorkspace.getHouseIconsContainer().isAnimateOn()){
            ...
            if(alreadyOnHome && mState == State.HOUSEWORKSPACE && !mHouseWorkspace.isTouchActive()){
            	mHouseWorkspace.moveToDefaultScreen(true);
            }
            closeFolder();
            exitSpringLoadedDragMode();
            showWorkspace(alreadyOnHome);
            showHouseWorkspace(alreadyOnHome);
    ...
    public void onDestroy() {
        ...
        mHouseWorkspace.removeCallbacks(mBuildLayersRunnable);
        ...
        mWorkspace = null;
        //caoshiyu add
        mHouseWorkspace.removeAllViews();
        mHouseWorkspace = null;
        ...
        //weather, added by shi yunlong
//        stopService(new Intent(this, WeatherProviderService.class));
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(MENU_GROUP_SCENE_CLASSIC, MENU_SCENE_CLASSIC, 0, R.string.classic_scene)
        ...
        menu.add(MENU_GROUP_SCENE_HOUSE, MENU_SCENE_HOUSE, 0, R.string.house_scene)
        .setIcon(android.R.drawable.ic_menu_preferences)
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mOnlyAppsPane.isTransitioning()){
        	return false;
        }
        if(isHouseWorkspace() && mAddAppsToHouseWorkspace != null 
        		&& mAddAppsToHouseWorkspace.isInShowcase()){
        	return false;
        }
        /* added by hct lijunyi, 20130130 */
        if (mBooksSelector.getVisibility() == View.VISIBLE) {
        ...
        if(isHouseWorkspace() && mHouseWorkspace.isZoom()){
        	zoomOutHouse(true);
        	return false;
        }
        boolean allAppsVisible = (mAppsCustomizeTabHost != null&&
        		mAppsCustomizeTabHost.getVisibility() == View.VISIBLE);
        menu.setGroupVisible(MENU_GROUP_WALLPAPER, !allAppsVisible && isDefaultWorkspace());
        //caoshiyu
        menu.setGroupVisible(MENU_GROUP_SCENE_CLASSIC, !isDefaultWorkspace());
        menu.setGroupVisible(MENU_GROUP_SCENE_HOUSE, !isHouseWorkspace()
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case MENU_WALLPAPER_SETTINGS:
            startWallpaper();
            return true;
		case MENU_SCENE_HOUSE:
//			String[] infos = getResources().getStringArray(
//					R.array.other_launcher_info);
//			startOtherLauncher(infos[0], infos[1]);
			return true;
        case MENU_SCENE_CLASSIC:
//        	if(isHouseWorkspace()){
//            	if(isAllAppsCustomizeOpen()&&mOnlyAppsPane.getVisibility() == View.VISIBLE){
////            		mOnlyAppsPane.setVisibility(View.INVISIBLE);
//            		hideOnlyAppsCustomizeHelper(false, false);
//            	}
//        		closeDoor();
//        	}
//			String[] infos = getResources().getStringArray(
...
//			startOtherLauncher(infos[0], infos[1]);
    ...
    //>>hct:add for start cooee launcher
    private void initHomeLauncherResolveInfos(){
		if(mHomeLauncherResolveInfos.size() == 0){
			Intent intent = new Intent(Intent.ACTION_MAIN);
			intent.addCategory(Intent.CATEGORY_HOME);
			mHomeLauncherResolveInfos = (ArrayList<ResolveInfo>) getPackageManager().queryIntentActivities(intent, 0);
			Log.d("caoshiyu", "mHomeLauncherResolveInfos---"+mHomeLauncherResolveInfos.size()+",mHomeLauncherResolveInfos="+mHomeLauncherResolveInfos);
		}
    }
    private void startOtherLauncher(String pkgName, String clsName){
    	PackageManager pm = getPackageManager();
    	initHomeLauncherResolveInfos();
    	final String localPkgName = getPackageName();
    	if(pkgName==null){
    		for (int i = 0; i < mHomeLauncherResolveInfos.size(); i++) {
    			ResolveInfo info = mHomeLauncherResolveInfos.get(i);
    			if(!(info.activityInfo.packageName.equals(localPkgName))){
    	...
    	pm.clearPackagePreferredActivities(localPkgName);
    	IntentFilter filter = new IntentFilter();
		filter.addAction(Intent.ACTION_MAIN);
		filter.addCategory(Intent.CATEGORY_HOME);
		filter.addCategory(Intent.CATEGORY_DEFAULT);
		ComponentName cn = new ComponentName(pkgName, clsName);
    	ComponentName oldCn = getComponentName();
		ComponentName[] set = new ComponentName[]{oldCn, cn };
		for (int i = 0; i < mHomeLauncherResolveInfos.size(); i++) {
			ResolveInfo info = mHomeLauncherResolveInfos.get(i);
			if(info.activityInfo.packageName.equals(pkgName)){
				Log.d("caoshiyu", "addPreferredActivity~~~"+pkgName+"info.match-->"+info.match);
				pm.addPreferredActivity(filter, info.match, set, cn);
				break;
			}
		}
...
//		intent.addFlags(Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED | Intent.FLAG_ACTIVITY_NEW_TASK);
		intent.putExtra("hct_launcher", true);
		intent.putExtra("hct_launcher_component", getComponentName());
    ...
    public void closeDoor(){
    	...
    	mDoor.runCloseWholeDoorAnim();
    }
    public void openDoor(){
    	mSwitchScene = true;
    	mDoor.setVisibility(View.VISIBLE);
    	mDoor.runDoorHandleAnim();

    }
    private Runnable mExitHouseRunnable = new Runnable() {
		@Override
		public void run() {
//    		exitOldSceneWorkspace(HOUSE_SCENE);
//    		switchSceneWorkspace(DEFAULT_SCENE);
//			if (mCompleteDoorRunnable != null) {
//				mHandler.postDelayed(mCompleteDoorRunnable, 1000);
//			}
			mCompleteDoorRunnable.run();
			String pkgName = null;
			String clsName = null;
			if(mOtherLauncherComponent != null){
				Log.d("caoshiyu-launcher2-scene2", "mOtherLauncherComponent-->"+mOtherLauncherComponent);
				pkgName = mOtherLauncherComponent.getPackageName();
				clsName = mOtherLauncherComponent.getClassName();
			}
			startOtherLauncher(pkgName,clsName);
			mHandler.postDelayed(new Runnable() {
				@Override
				public void run() {
					// TODO Auto-generated method stub
					mDoor.setVisibility(View.INVISIBLE);
					
				}
			}, 1000);
		}
	};
	private Runnable mCompleteDoorRunnable = new Runnable() {
		@Override
		public void run() {
			mSwitchScene = false;
		}
	};
    public void switchSceneWorkspace(String scene){
    	Bundle b = new Bundle();
    	b.putString("selectedScene", scene);
    	Intent intent = new Intent(LauncherModel.SWITCH_SCENE_ACTION);
    	intent.putExtras(b);
    	sendBroadcast(intent);
    }
    public void exitOldSceneWorkspace(String scene){
    	if(scene.equals(DEFAULT_SCENE)){
//    		mWorkspace.setLayerType(View.LAYER_TYPE_NONE, null);
    		if(isFolderClingVisible())dismissFolderCling(null);
    		if(isWorkspaceClingVisible())dismissWorkspaceCling(null);
    		...
    		mWorkspace.hideDotIndicator();
    		hideDockDivider();
    		if(mAppsCustomizeTabHost != null && isAllAppsCustomizeOpen() 
    				&& mAppsCustomizeTabHost.getVisibility() == View.VISIBLE){
//    			hideAppsCustomizeHelper(false, false);
...
//            }
    	...
    	}else if(scene.equals(HOUSE_SCENE)){
//    		mHouseWorkspace.setLayerType(View.LAYER_TYPE_NONE, null);
    		hideHouseStyleTransfer(false);
//    		mHouseWorkspace.removeAllViews();
    		mHouseWorkspace.hideDotIndicator();
    		hidePet();
    		if(isAllAppsCustomizeOpen() && mOnlyAppsPane.getVisibility() == View.VISIBLE){
    			mOnlyAppsPane.setVisibility(View.INVISIBLE);
    		}
    		mHouseWorkspace.changeState(HouseWorkspace.State.NORMAL, false);
    		mHouseWorkspace.setVisibility(View.INVISIBLE);
    	}
//    	if(isAllAppsVisible() && mAppsCustomizeTabHost.getVisibility() == View.VISIBLE){
//    		mAppsCustomizeTabHost.setVisibility(View.GONE);
//    	}
    }
    public void entryNewSceneWorkspace(String scene){
    	Log.d("furniture", "entryNewSceneWorkspace--scene=="+scene);
    	if(scene.equals(DEFAULT_SCENE)){
            ...
            mWorkspace.showDotIndicator();
            mWorkspace.adjustDotIndicator();
    	}else if(scene.equals(HOUSE_SCENE)){
    		showHouseWorkspace(false);
    		mHouseWorkspace.setVisibility(View.VISIBLE);
    		mHouseWorkspace.showDotIndicator();
    		mHouseWorkspace.adjustDotIndicator();
    		showPet();
    	}
    }
    @Override
    ...
    public boolean isHouseWorkspaceLocked(){
    	return mHouseWorkspaceLoading;
    }
...
//        }
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch (event.getKeyCode()) {
                ...
                case KeyEvent.KEYCODE_MENU:
                	if(isHouseWorkspace() && mOnlyAppsContent.isShake()){
                		mOnlyAppsContent.startShakeAnimate(false);
                		return true;
                	}
                	break;
    ...
    public void onBackPressed() {
        ...
        if (mState == State.APPS_CUSTOMIZE) {
        	if(isDefaultWorkspace()){
        		...
        		if(!mOnlyAppsContent.isShake()){
        			showHouseWorkspace(true);
        		}
        	}
        } else if(mState == State.HOUSEWORKSPACE){
        	if(mHouseWorkspace.isZoom()){
        		zoomOutHouse(true);
        	}else{
        		if(mHouseWorkspace.getHouseIconsContainer() != null
						&&mHouseWorkspace.getHouseIconsContainer().isAnimateOn())
        			mHouseWorkspace.getHouseIconsContainer().startShakeAnimate(false);
        	}
        ...
        /* added by hct, 20121011 */
        if (View.VISIBLE ==  mScreenPreview.getVisibility()){
        	mScreenPreview.previewExit();
        }
        /* end of added by hct, 20121011 */

        if (isHouseWorkspace()) {
            hideBookSelector();  /* added by hct lijunyi, 20130130 */
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (mHouseWorkspace.isSwitchingState()) {
        	return;
        }
        if(isHouseWorkspace()){
        	if (mSwitchScene) {
        	...
        	if(v.getParent() == mPetFurniture && !mSwitchScene){
        		//pet
        		showOnlyAllApps(true);
        		return;
			} else if (v instanceof HouseCellLayout) {
				if (mHouseWorkspace.isZoom()) {
					Log.d("furniture", "click on HouseCellLayout");
					mHouseStyleTransfer.setFocusView(mHouseWorkspace);
					mHouseStyleTransfer.updateStyle(
							getResources().getString(R.string.wall),
							R.array.house_wall, getHouseWall());
					return;
				}
			}else if (v instanceof SceneShelfItem){
				if(mHouseWorkspace.isZoom()){
        			mHouseWorkspace.startCatchBeforeExit(v);
				}else if(!mHouseWorkspace.getHouseIconsContainer().isAnimateOn()){
					ShortcutInfo tag = (ShortcutInfo) v.getTag();
					final Intent intent = tag.intent;
					startActivitySafely(intent, tag);
				}
				return;
			}else if (v.getParent() instanceof SceneShelfItem 
					&& v.getTag() instanceof ShortcutInfo){
				//remove the house icons 
				ArrayList<ShortcutInfo> removeList = new ArrayList<ShortcutInfo>(1);
				removeList.add((ShortcutInfo)v.getTag());

				mHouseWorkspace.getHouseIconsContainer().removeShortcutInfo(removeList);
        ...
        Object tag = v.getTag();
        /*chengzhi: add the preshortcut 20121128*/
        if (tag instanceof PreShortcutInfo) {
			int cell_x = ((PreShortcutInfo) tag).cellX ;
			int cell_y = ((PreShortcutInfo) tag).cellY ;
			int mindex = 0;
			String title = (String) ((PreShortcutInfo) tag).title ;
			Intent preIntent = ((PreShortcutInfo) tag).intent;
			
/*			Log.v(TAG, "Launcher-onClick PreShortcutInfo cell_x ="
					+ cell_x +", cell_y = " + cell_y + ",title = " + title
					+ ",preIntent = " + preIntent);*/
		
			String[] apk_name = getResources().getStringArray(R.array.pre_apk_filename) ;
			String[] apk_packname = getResources().getStringArray(R.array.pre_apk_packname) ;
			String preShortpgname = preIntent.getComponent().getPackageName();
//			Log.v(TAG, "Launcher preShortpgname == "+ preShortpgname);
//			Log.v(TAG, "Launcher apk_packname_0 == "+ apk_packname[0]);
//			Log.v(TAG, "Launcher apk_packname_1 == "+ apk_packname[1]);
			
			for (int i = 0; i < apk_packname.length; i++) {
				if(apk_packname[i].equals(preShortpgname)){
					mindex = i;
					break;
				}
			}
				
			PreShortcutInstaller preShortcutInstaller = new PreShortcutInstaller(getApplicationContext(),preIntent);	

			preShortcutInstaller.installApk(apk_name[mindex]) ;	         	
...
//        }
    	Intent intent = new Intent(Intent.ACTION_MAIN);
		String[] marketPkg = getResources().getStringArray(R.array.market_app);
    	if(marketPkg.length == 2){
    		intent.setClassName(marketPkg[0], marketPkg[1]);
//    		startActivitySafely(intent, "app market");
...
//        if(mState == State.HOUSEWORKSPACE){
//        	Log.d("furniture", "launcher--houseWork--v"+v);
//        	
//        	return true;
//        }
        if (mState != State.WORKSPACE && isDefaultWorkspace()) {
            return false;
        }
        ...
        if(isHouseWorkspace()){
        	...
        	boolean allowLongPressInHouse = mHouseWorkspace.allowLongPress() && !mHouseWorkspace.isZoom();
        	if(allowLongPressInHouse && !mDragController.isDragging()){
        		if(v instanceof HouseCellLayout){
        			Log.d("furniture", "longclick0000----v=="+v);
        			mHouseStyleTransfer.setFocusView(mHouseWorkspace);
        			mHouseStyleTransfer.updateStyle(getResources().getString(R.string.wall),
        					R.array.house_wall, getHouseWall());
        		}else if(v instanceof SceneShelfItem && !mHouseWorkspace.isZoom()){
        			//long click to shake icon
        			mHouseWorkspace.getHouseIconsContainer().startShakeAnimate(true);
        			return true;
        		}else{
        			Log.d("furniture", "longclick1111----v=="+v);
        			while(!(v instanceof Furniture)){
        				v = (View)v.getParent();
        			}
        			mHouseWorkspace.startCatch(v);
        		}
        		mHouseWorkspace.performHapticFeedback(HapticFeedbackConstants.LONG_PRESS,
        		...
        		if(itemUnderLongClick == null){
        			Log.d("furniture", "longclick0000----v=="+v);
        			mHouseStyleTransfer.updateStyle(getResources().getString(R.string.wall),
        					R.array.house_wall, 0);
        		}else{
        			Log.d("furniture", "longclick1111----v=="+v);
        			mHouseWorkspace.startCatch(longClickCellInfo);
        		}
        		 */
        ...
        if (longClickCellInfo == null) {
        	Log.d("furniture", "longClickCellInfo  is null!!!");
            return true;
        }
...
//                	mWorkspace.startDragAppWidget(mWorkspace.getCurrentPage());
    ...
    HouseWorkspace getHouseWorkspace(){
    	return mHouseWorkspace;
    }
    HouseStyleTransfer getHouseStyleTransfer(){
    	return mHouseStyleTransfer;
    }
    Furniture getPetFurniture(){
    	return mPetFurniture;
    }
    public void showPet(){
    	if(mPetFurniture != null){
    		mPetFurniture.setVisibility(View.VISIBLE);
    	}
    }
    public void hidePet(){
    	if(mPetFurniture != null){
    		mPetFurniture.setVisibility(View.GONE);
    	}
    }
    //sharedPreferences for house wall
    public void saveHouseWall(int style){
    	SharedPreferences settings = getSharedPreferences(HOUSE_WALL, 0);
    	SharedPreferences.Editor editor = settings.edit();
    	editor.putInt(HOUSE_WALL, style);
    	editor.commit();
    }
    public int getHouseWall(){
    	return getSharedPreferences(HOUSE_WALL, 0).getInt(HOUSE_WALL,
    			getResources().getInteger(R.integer.config_house_wall_style));
    }
    @Override
    ...
    /**
     *  add for house apps paged view
     * 
     * */
    
    private void showOnlyAppsCustomizeHelper(boolean animated, final boolean springLoaded) {
        ...
        final View toView = mOnlyAppsPane;
        final int startDelay =
                res.getInteger(R.integer.config_workspaceAppsCustomizeAnimationStagger);
        
        final View contentView = mOnlyAppsContent;
...
//                    toView.setScaleY(a * scale + b * 1f);
                	toView.setScaleX(a * 0.2f + b * 1.0f);
                	toView.setScaleY(a * 0.2f + b * 1.0f);
//                	contentView.setTranslationY(0.5f*contentView.getMeasuredHeight() * a + b*0.0f);
                }
            });
    ...
    private void hideOnlyAppsCustomizeHelper(boolean animated, final boolean springLoaded) {
        if (mStateAnimation != null) {
            mStateAnimation.cancel();
            mStateAnimation = null;
        }
        ...
        final View fromView = mOnlyAppsPane;
        final View contentView = mOnlyAppsContent;
        ...
        if (animated) {
            final float oldScaleX = fromView.getScaleX();//contentView.getScaleX();
            final float oldScaleY = fromView.getScaleY();//contentView.getScaleY();
//        	final float oldTranslateX = tabContentView.getTranslationX();
//        	final float oldTranslateY = tabContentView.getTranslationY();
            ...
            scaleAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
                	fromView.setScaleX(a * oldScaleX + b * 0.2f);
                	fromView.setScaleY(a * oldScaleY + b * 0.2f);
//                	tabContentView.setTranslationY(a * oldTranslateY + b * tabContentView.getMeasuredHeight() * 0.5f);
                }
            });
...
//                    tabContentView.setAlpha(a * 1f + b * 0f);
                }
            });
...
//            	mHouseWorkspace.setVisibility(View.VISIBLE);
    ...
    private void showAppsCustomizeHelper(boolean animated, final boolean springLoaded) {
        ...
        final View tabContentView = mAppsCustomizeTabHost.getTabContentView();
        final View tabWidget = mAppsCustomizeTabHost.getTabWidget();
        setPivotsForZoom(toView, scale);

        if(mLauncherStart){
        	mLauncherStart = false;
//        	mWorkspace.setVisibility(View.INVISIBLE);
...
//                    toView.setScaleX(a * scale + b * 1f);
//                    toView.setScaleY(a * scale + b * 1f);
                	tabContentView.setScaleX(a * 0.7f + b * 1.0f);
                	tabContentView.setScaleY(a * 0.7f + b * 1.0f);
                	tabContentView.setTranslationY(0.5f*tabContentView.getMeasuredHeight() * a + b*0.0f);
                }
            });
            toView.setVisibility(View.VISIBLE);
            toView.setAlpha(0f);
            ...
            alphaAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
                    // don't need to invalidate because we do so above
                    toView.setAlpha(a * 0f + b * 1f);
                	tabWidget.setAlpha(a * 0f + b * 1f);
                    tabContentView.setAlpha(a * 0f + b * 1f);
                }
            });
            alphaAnim.setStartDelay(startDelay);
            alphaAnim.start();
            ...
            scaleAnim.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    toView.setScaleX(1.0f);
                    toView.setScaleY(1.0f);
                    tabContentView.setTranslationY(0.0f);
                    tabContentView.setAlpha(1.0f);
                    if (toView instanceof LauncherTransitionable) {
                        ((LauncherTransitionable) toView).onLauncherTransitionEnd(instance,
                                scaleAnim, false);
                    }
...
//        showHotseat(animated);
        if (animated) {
            final float oldScaleX = tabContentView.getScaleX();
            final float oldScaleY = tabContentView.getScaleY();
//        	final float oldTranslateX = tabContentView.getTranslationX();
        	final float oldTranslateY = tabContentView.getTranslationY();
            ValueAnimator scaleAnim = ValueAnimator.ofFloat(0f, 1f).setDuration(duration);
            scaleAnim.setInterpolator(new Workspace.ZoomInInterpolator());
            scaleAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
                	tabContentView.setScaleX(a * oldScaleX + b * 0.7f);
                	tabContentView.setScaleY(a * oldScaleY + b * 0.7f);
                	tabContentView.setTranslationY(a * oldTranslateY + b * tabContentView.getMeasuredHeight() * 0.5f);
                }
            });
            final ValueAnimator alphaAnim = ValueAnimator.ofFloat(0f, 1f);
...
//            mWorkspace.setVisibility(View.VISIBLE);
	...
	void showHouseStyleTransfer(boolean animated) {
		if (mHouseStyleTransfer != null) {
			mHouseStyleTransfer.setVisibility(View.VISIBLE);
			/*
			if (mHouseStyleTransferAnimator != null) {
				mHouseStyleTransferAnimator.cancel();
				mHouseStyleTransfer.setAlpha(1f);
				mHouseStyleTransferAnimator = null;
			}
			if (animated) {
				mHouseStyleTransferAnimator = new AnimatorSet();
				mHouseStyleTransferAnimator.play(
						ObjectAnimator.ofFloat(mHouseStyleTransfer, "alpha", 1f));
				mHouseStyleTransferAnimator.setDuration(250);
				mHouseStyleTransferAnimator.start();
			}
			*/
		}
	}
    void hideHouseStyleTransfer(boolean animated){
    	if(mHouseStyleTransfer != null){
    		if(animated){
    		...
    		mHouseStyleTransfer.setVisibility(View.GONE);
    	}
    }
    void showHouseWorkspace(boolean animated){
    	if(!isHouseWorkspace()) return;
    	Log.d("furniture", "showHouseWorkspace~mState=="+mState);
    	if(mHouseWorkspace.isZoom()){
    		zoomOutHouse(animated);
    	}
    	mHouseWorkspace.changeState(HouseWorkspace.State.NORMAL, true);
		if(mState != State.HOUSEWORKSPACE){
			//hide all apps
			if(mState == State.APPS_CUSTOMIZE){
//				hideAppsCustomizeHelper(animated, false);
				hideOnlyAppsCustomizeHelper(animated, false);
			}
//			mHouseWorkspace.setVisibility(View.VISIBLE);
		}
		mState = State.HOUSEWORKSPACE;
    ...
    void zoomInHouse(boolean animated){
    	if(mHouseWorkspace != null){
    		mHouseWorkspace.getHouseIconsContainer().startShakeAnimate(false);
    		mHouseWorkspace.changeState(HouseWorkspace.State.ZOOM, animated);
    		hidePet();
    		showHouseStyleTransfer(animated);
    	}
    }
    void zoomOutHouse(boolean animated){
    	Log.d("furniture", "zoomOutHouse~~~");
    	if(mHouseWorkspace != null){
    		mHouseWorkspace.changeState(HouseWorkspace.State.NORMAL, animated);
    		showPet();
    		hideHouseStyleTransfer(animated);
    		mHouseWorkspace.exitCatch();
    	}
    }
    void showOnlyAllApps(boolean animated){
    	if(mHouseWorkspace.getHouseIconsContainer() != null)
    		mHouseWorkspace.getHouseIconsContainer().startShakeAnimate(false);
        showOnlyAppsCustomizeHelper(animated, false);
        mOnlyAppsPane.setFocusable(true);
        mOnlyAppsPane.requestFocus();
        // Change the state *after* we've called all the transition code
	...
	void showAllApps(boolean animated) {
        ...
        if(isDefaultWorkspace()){
        	...
        	if(mWorkspace.getCurrentDropLayout() != null)
...
//        	mWorkspace.startCovered(mWorkspace.getCurrentPage());
    ...
    void enterSpringLoadedDragMode() {
        if (mState == State.APPS_CUSTOMIZE && isDefaultWorkspace()) {
    ...
    private boolean updateGlobalSearchIcon() {
    	if(mSearchDropTargetBar.isQSBSearchBarNull()) return false;
    ...
    private void updateGlobalSearchIcon(Drawable.ConstantState d) {
    	if(mSearchDropTargetBar.isQSBSearchBarNull()) return;
        final View searchButtonContainer = findViewById(R.id.search_button_container);
...
//        final TextView marketButton = (TextView) findViewById(R.id.market_button);
//        marketButton.setCompoundDrawablesWithIntrinsicBounds(null, top, right, bottom)
//        marketButton.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.ic_launcher_market_holo, 0, 0);
...
//        updateTextButtonWithDrawable(R.id.market_button, d);
    ...
    public void startBinding() {
		...
		if(true){//(isDefaultWorkspace()){
			// remove all child in all celllayouts of workspace
			final Workspace workspace = mWorkspace;
			...
			for (int i = 0; i < count; i++) {
				...
				final CellLayout layoutParent = (CellLayout) workspace
						.getChildAt(i);
				if(layoutParent.getChildrenChildCount() > 0){
		...
		if(true){//(isHouseWorkspace()){
			// remove all child in all celllayouts of workspace
			final HouseWorkspace houseWorkspace = mHouseWorkspace;
			mHouseWorkspace.clearDropTargets();
			int count = houseWorkspace.getChildCount();
			for (int i = 0; i < count; i++) {
				// Use removeAllViewsInLayout() to avoid an extra requestLayout()
				// and invalidate().
				final CellLayout layoutParent = (CellLayout) houseWorkspace
						.getChildAt(i);
				if(layoutParent.getChildrenChildCount() > 0){
					layoutParent.removeAllViewsInLayout();
					layoutParent.requestChildLayout();
			...
			if(houseWorkspace.getChildCount()>0)
				houseWorkspace.invalidate();
		}
        if(isDefaultWorkspace()){
        	mWidgetsToAdvance.clear();
        	if (mHotseat != null) {
...
//        if(!isDefaultWorkspace()) return;
        //caoshiyu add for hide the dot indicator
    		...
    		if(isHouseWorkspace()){
    	        ...
    	        for (int i=start; i<end; i++) {
    	            ...
    	            switch (item.itemType) {
    	                case LauncherSettings.Favorites.ITEM_TYPE_FURNITURE:
    	                    View furniture = createFurniture((FurnitureInfo)item);
    	                    houseWorkspace.addInScreen(furniture, item.container, item.screen, 
    	                    		item.cellX, item.cellY, item.spanX, item.spanY, false);
    	                    break;
    	                case LauncherSettings.Favorites.ITEM_TYPE_APPLICATION:
    	                	View houseShortcut = createHouseShortcut((ShortcutInfo)item);
    	                	houseWorkspace.addInShelf(houseShortcut, item, false);
    	                	break;
    	            }
    	        }
    	        houseWorkspace.requestLayout();
    		}
    		return;
    	} 
        if (LauncherLog.DEBUG) {
        ...
        for (int i=start; i<end; i++) {
            ...
            switch (item.itemType) {
                ...
                case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                    ...
                    break;
                /*chengzhi: add the preshortcut 20121128*/    
                case LauncherSettings.Favorites.ITEM_TYPE_PRE_SHORTCUT:
					View preShortcut = createShortcut((PreShortcutInfo)item);
                    workspace.addInScreen(preShortcut, item.container, item.screen, item.cellX,
                            item.cellY, 1, 1, false);  
                    break;
    ...
    public void finishBindingItems() {
        ...
        if(!isDefaultWorkspace()){
        	if(isHouseWorkspace()){
        		...
        		if(mHouseWorkspace.getDotIndicatorView()!= null){
        			mHouseWorkspace.initDotIndicator();
        			mHouseWorkspace.adjustDotIndicator();
        			mHouseWorkspace.updateCurrentDotIndicator();
        		}
        		mHouseWorkspace.setCurrentPage(mHouseWorkspace.getCurrentPage());
        		mHouseWorkspaceLoading = false;
        	}
        	return;
        }
        */ 
        ...
        if (mSavedState != null) {
            if (!mWorkspace.hasFocus() && isDefaultWorkspace()) {
                mWorkspace.getChildAt(mWorkspace.getCurrentPage()).requestFocus();
            }
            if (!mHouseWorkspace.hasFocus() && isHouseWorkspace()) {
            	mHouseWorkspace.getChildAt(mHouseWorkspace.getCurrentPage()).requestFocus();
            }
            mSavedState = null;
        }
...
//        if(isHouseWorkspace())
        	mHouseWorkspace.post(mBuildLayersRunnable);
...
//            findViewById(R.id.apps_customize_progress_bar);
        ...
        if(isHouseWorkspace()){
        	mOnlyAppsPane.post(new Runnable() {
        		@Override
        		public void run() {
        			if(mOnlyAppsContent != null){
        				mOnlyAppsContent.setApps(apps);
//        				 if (progressBar != null) {
//                             ((ViewGroup)progressBar.getParent()).removeView(progressBar);
//                         }
        			}
        	...
        	mBooksSelector.post(new Runnable() {
				@Override
				public void run() {
					// TODO Auto-generated method stub
		            ((BooksDisplay)mBooksSelector.getLayoutShow()).setApps(apps);  /* added by hct lijunyi, 20130204 */
				}
			});
        }
...
//                    }
    ...
    public void bindAppsAdded(ArrayList<ApplicationInfo> apps) {
        ...
        if (null != mBooksSelector) {
        	 ((BooksDisplay)mBooksSelector.getLayoutShow()).addApps(apps);
        }
    }
    ...
    public void bindAppsUpdated(ArrayList<ApplicationInfo> apps) {
        ...
        if(mHouseWorkspace != null){
        	mHouseWorkspace.updateShortcuts(apps);
        }
        if (mOnlyAppsContent != null) {
        	mOnlyAppsContent.updateApps(apps);
        }
        if (mAppsCustomizeContent != null) {
        ...
        if (null != mBooksSelector) {
        	((BooksDisplay)mBooksSelector.getLayoutShow()).updateApps(apps);
        }
    }
    ...
    public void bindAppsRemoved(ArrayList<ApplicationInfo> apps, boolean permanent) {
        ...
        if (permanent) {
            ...
            //caoshiyu add:
            mHouseWorkspace.removeItems(apps);
        }

        if (null != mBooksSelector) {
        	 ((BooksDisplay)mBooksSelector.getLayoutShow()).removeApps(apps);
        }

        if (mOnlyAppsContent != null) {
        	mOnlyAppsContent.removeApps(apps);
        }
        if (mAppsCustomizeContent != null) {
    ...
    /* Cling related */
    private static final String PREFS_KEY = "com.hct.scene.launcher2.prefs";
            ...
            anim.addListener(new AnimatorListenerAdapter() {
                public void onAnimationEnd(Animator animation) {
                    ...
                    SharedPreferences prefs =
                        getSharedPreferences("com.hct.scene.launcher2.prefs", Context.MODE_PRIVATE);
    ...
    public static boolean isHouseWorkspace(){
    	return mCurrentScene.equals(HOUSE_SCENE);
    }
    public static boolean isDefaultWorkspace(){
    	return mCurrentScene.equals(DEFAULT_SCENE);
    }
    ...
    //add by hct chenxin
    public void setThemeWallpaer(String name){
    
    	mWorkspace.setWallpaper(name);
    }
    private void switchScene(String scene) {
    	...
    	*/
        ...
        if (!LauncherModel.exists(this, scene)){
//        if(LauncherModel.hasLoadFavorites(this)){
        	dbHelper.loadFavorites(db, getSceneDefinedIdBySceneName(scene));
//        	LauncherModel.setLoadFavorites(this, false);
        }
...
//    	mWorkspace.setWallpaper(mCurrentWallpaper);
    ...
    public void bindComponentUnreadChanged(final ComponentName component, final int unreadNum) {
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                if (mHouseWorkspace != null) {
                	mHouseWorkspace.updateComponentUnreadChanged(component, unreadNum);
                }
   ...
   /**
    ...
    private void bindWorkspaceUnreadInfo() {
        mHandler.post(new Runnable() {
            public void run() {
                ...
                if (mHouseWorkspace != null) {
                	mHouseWorkspace.updateShortcutsAndFoldersUnread();
                }
                if (LauncherLog.DEBUG_PERFORMANCE) {
	...
	/* added by hct, 20121011 */
	void hideAllView(boolean flags) {
		if (flags) {
			mSearchDropTargetBar.setVisibility(View.INVISIBLE);
			mWorkspace.setVisibility(View.INVISIBLE);
			mWorkspace.hideDotIndicator();
...
//			mDockDivider.setVisibility(View.VISIBLE);
	...
	public Workspace getWorkspace(boolean flag) {
		return mWorkspace;
	}

	public void showPreviews() {
		if(isHouseWorkspace()) return;
		if (mScreenPreview.getVisibility() == View.VISIBLE) {
			return;
		}

		if (isWorkspaceLocked() == true) {
			return;
		}
		...
		try {
			for (int i = 0; i < mWorkspace.getChildCount(); i++) {
				...
				final Bitmap bitmap = Bitmap.createBitmap(cell.getWidth(),
						cell.getHeight(), Bitmap.Config.ARGB_8888);
		...
		mScreenPreview.showPreview(bitmaps, getScreenCountMax());
	}

	public void bindScreens(int screen_num)
	{
		final Workspace workspace = mWorkspace;
		...
		workspace.initScreens(screen_num);
	}
	
	public int getScreenCountMax(){
		return WORKSPACE_MAX_PAGE_NUM;
	}
	...
	public void adjustAllPages(ArrayList<Integer> pageOrder){
		if(pageOrder.size()<1)return;

		SharedPreferences.Editor edit = getSharedPreferences(WS_SCREEN_BASE, 0).edit();
		edit.putInt(KEY_WS_SCREEN_NUM, pageOrder.size());
		edit.commit();
		mWorkspace.adjustAllPages(pageOrder);
	}

	public boolean checkPageIsEmpty(int order){
		CellLayout page = (CellLayout)mWorkspace.getChildAt(order); 

		if(page==null || page.getChildrenChildCount()==0){
			return true;
		}
		return false;
	}
	...
	private AddAppsToHouse mAddAppsToHouseWorkspace = new AddAppsToHouse();

	class AddAppsToHouse implements Runnable{
		private View v;
		private boolean mInShowcase;
		public void setView(View v){
			this.v = v;
		}
		public boolean isRefused(){
			return mHouseWorkspace.getHouseIconsContainer().isAnimateOn() || mSwitchScene;
		}
		public void setInShowcase(boolean flag){
			mInShowcase = flag;
		}
